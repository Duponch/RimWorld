...
État : 14 septembre 2026. Le statut technique ci-dessous fait autorité ; la matrice de recherche décrit des cibles de gameplay, pas des fonctionnalités déjà disponibles.
...
| Sauvegarde matérielle et besoins | Schéma 9 livré | Transport, dégagement agricole, propriété, ingestion, couchage, trajets et progression persistés ; migrations V1–V8 explicites. Manifeste et journal complet restent ouverts. |
...
| Présentation 3D | Livrée avec placeholders | Décor fusionné par chunks, retrait de ressources par indices conservés, lots persistants ; rig rigide huit os GPU TSL. Vue distante instanciée sans reconstruction au zoom, marche linéaire et orientation vers les actions. Caméra bornée au terrain, bouton iso/perspective et cycle visuel jour/nuit ; climat de gameplay à venir. |
...
| Agriculture | Première boucle de riz livrée, G1 partiel | Champ, dégagement des piles, semis sans graines, défrichage, croissance avec lumière, récolte mûre, second semis, réglages et priorité Culture ; autres espèces, climat et cuisine absents. Voir [contrat](development/farming.md). |
| Organisation de l'interface | Livrée sur le périmètre actuel | Disposition RimWorld, Architecte avec réserves, lits/tables orientés et tabourets ; compteur FPS permanent, Travail collecte/construction/transport/culture 0–4, inspection des matériaux et filtres. Les autres onglets restent désactivés jusqu'à leurs mécaniques. |
...
| Validation | V9 ciblée | Contrats matériels/agricoles/alimentaires, oracle sur 120 cartes, colonies sur trois graines, trois parcours UI courts et reprise réussie du parcours de trois jours ; premier blocage navigateur et pointes CPU du stress conservés dans les [preuves](development/validation.md). |
| Fidélité au jeu de base complet | En développement | Petite partie du socle : agriculture de riz livrée, cuisine, combat et storyteller encore absents. Voir l’inventaire par domaine pour les autres manques. |

## Présentation jour/nuit et deux projections — après V7

[Contrat livré](development/daylight-camera.md) : bouton iso/perspective, cadrage et pointage conservés, ciel TSL et éclairage synchronisés à l’heure sauvegardée. [Audit rétroactif](research/environment-review.md) : plusieurs roches, sols, espèces et biomes restent à développer ; aucun domaine n’est clos par son seul rendu. Le ciel clair fixe ne clôture pas météo/climat G2. Sauvegardes V7 conservées.
...
**Tranche livrée :** cultures, dégagement physique des piles gênantes et choix alimentaire du profil adulte neutre, G1 partiel, schéma 9. Voir [règles vérifiées](research/food-clearing-reference.md). **Prochaine tranche :** cuisine/postes/recettes et ingrédients physiques, après nouvelle recherche. Consolider en parallèle les réservations de cases de travail G0. Météo, saisons et toits restent G2. Le preset de croissance ne vaut pas simulation de ces systèmes.

L’audit V9 a conduit à cibler les recherches de travaux et de livraisons. La charge agricole de trois colons est améliorée ; les pointes CPU du stress à trente cultivateurs restent ouvertes en G0. Au prochain audit de navigation, isoler les replis de recherche et les allocations/GC avant de choisir une nouvelle optimisation. Les [mesures](development/validation.md) ne constituent pas une garantie de fluidité sur toute machine.

## Priorité : interactions de survie, puis consolidation G0

La remarque utilisateur sur les raccourcis élémentaires avance les vrais repas et couchages de G1 avant les zones nommées. [Cette tranche](development/needs.md) supprime consommation à distance et bonus de lit voisin, avec attribution et sauvegarde des actions. [Les repas à table](development/dining.md) ajoutent mobilier, réservations, confort et premier souvenir. **Aliments distincts, quantités et nutrition adulte sont livrés ([contrat](development/food-items.md)). Les buissons renouvelables V7 et le premier champ de riz V8 sont livrés ; choix alimentaire neutre et dégagement agricole V9 sont livrés ; régimes, horaires et cuisine restent ouverts** ; G0 continue avec zones nommées, ordres contextuels et congestion. La [relecture de la collecte](research/colony-progression.md) avait identifié la destruction du buisson récolté comme un écart, corrigé en V7. Aucune interaction fondamentale ne doit devenir un simple ajustement de jauge pour aller plus vite.
...
| Persistance | Schéma 9, migrations V1–V8, cargaison, chantier partiel, places de repas, confort et phases de besoins restaurés, données invalides refusées. | Journal complet des commandes datées et de leurs résultats, manifestes, export/import et reprise d'évolution des règles. |
