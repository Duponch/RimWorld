...
Incréments du lot : [requêtes de ligne/couvert/visée](development/combat-queries.md), [profils du revolver et précision](research/ranged-statistics-reference.md), puis [impacts V54](development/bullet-impact.md). Ces derniers ont leurs scénarios, migration stricte, clinique UI et mesure CPU à cent dossiers ; les impacts des fixtures sont injectés explicitement. Restent les propriétés tactiques du décor, le vrai producteur d'XP/Tir, les phases/vol sauvegardés et l'adversaire. Ces sous-étapes ne sont pas des livraisons de combat jouable.
