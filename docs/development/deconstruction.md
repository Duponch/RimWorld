...
`world.deconstructed` contient `count`, `lostWood`, `fuelTicks` et, depuis V30, `lostSteel?`, puis `lostBlocks?` par pierre en V33. L’acier perdu complète son propre bilan ; les restitutions conservent le matériau de la recette de l’ouvrage, historique ou actuelle. Les pertes ne sont pas des stocks disponibles : lostWood comptabilise le bois non rendu ; uelTicks conserve le combustible brûlé **et restant** retiré avec les feux. Un feu construit transforme déjà son coût initial en combustible, donc ne pas le compter deux fois. Le bilan du pilote est `bois présent + bois des arbres + ouvrages + combustible des feux + lostWood + fuelTicks / 600`. La perte ne crée aucun objet transportable.
