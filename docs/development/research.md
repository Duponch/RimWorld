...
- `research.ts` possède sélection, proposition, contribution et fin. `World.research` conserve projet, micro-points entiers (un million par point) et date d'achèvement. Vêtements complexes coûte 600 points ; Climatisation 500. `airConditioning` conserve ses propres points/date, distincts des champs historiques `points/completedAt`. Suspendre ou changer de projet conserve les deux progrès. V75 valide strictement V74 avant migration sans recherche inventée. Achèvement unique, notification, puis déblocage du tailleur ou du climatiseur selon le projet. [Chaîne du froid](cold-store.md). Les besoins, ordres, urgences, mobilisation et incapacités réutilisent les interruptions communes.
...
Choix de scénario assumé : départ sans Vêtements complexes, contrairement au Crashlanded classique. Propreté intérieure/difficulté/technologie neutres, coefficient extérieur incertain et choix des bureaux par proximité documentés dans la recherche liée. Deux projets ; bases électriques connues dans notre départ, sans arbre intermédiaire inventé. Pas de livres, analyseur, bureau électrique ni arbre complet. Les dégâts/qualités du mobilier autres que murs/portes/climatiseurs, nouveaux vêtements thermiques, usure quotidienne et tenues automatiques restent distincts.
