...
V55, [projectiles persistants](projectiles.md) : noyau pur conservé ; `projectile-state/save/system` gèrent enveloppe, validation/migration et sous-pas dans World. `projectile-batch` partage le décor fixe pendant la seule transaction synchrone et renouvelle les cibles mobiles après chaque impact médical. Aucun cache inter-tick ni rendu modifiant World. Le bridge observe émission/arrivée/retrait et supprime les champs dynamiques absents. Pas encore de commande/XP, phases de tireur, adversaire ou rendu de projectile ; dommages au décor et factions restent ouverts.
