...
Sous V54, [noyau de projectile](projectiles.md) : `bullet-emission` choisit la branche, `projectile-rules` résout les permissions/probabilités, `bullet-flight` avance les sous-pas sur une scène possédée par l'appelant. PRNG explicite, entrées copiées, arrivée unique. `projectile-world` capture maintenant les candidats actuels et expose des vues de relations copiées, avec colonnes numériques, incidences et identités à la demande ; la capture du meilleur couvert reste distincte. Aucun DOM/rendu ni engagement de vol par World ; le schéma 54 reste inchangé. Dépouilles, dommages aux objets et relations de factions persistantes restent ouverts.
