...
## Relecture de cadence V52

La garde obligatoire retrouve deux attentes brèves en minage, après changements de vitesse. Les délais de boutons restent sous 46 ms, sans saut ni retrait anticipé. La trace montre un autre défaut : le taux reçu entre deux frames s’appliquait à tout leur intervalle, y compris au temps antérieur à sa réception. Avec des durées asymétriques, ces petites erreurs vidaient progressivement le tampon.

Le test précédent changeait toujours de vitesse à la frontière exacte de ses frames et son oracle utilisait aussi ce nouveau taux sur l’intervalle écoulé. Un nouveau témoin prend deux confirmations entre des frames retardées et une réception datée après le prochain RAF : il échoue sur le code précédent. Le rendu intègre désormais les sous-intervalles horodatés, toujours seulement dans RAF, sans augmenter la réserve, extrapoler, sauter le curseur ou retarder le contrôle d’un passé tamponné. L’oracle long intègre indépendamment les intervalles à leur ancien taux avant confirmation. L’échec natif et les résultats de reprise restent dans la validation, pas effacés par un passage favorable.

Un intervalle qui chevauche une confirmation change déjà visiblement de vitesse, même si son taux moyen se situe entre ancien et nouveau. L’observateur conserve donc deux délais : premier effet visible (budget 100 ms) et première frame entière au nouveau taux. La détection du premier effet refuse immobilité, dépassement, taux inchangé et données invalides. Cela ne retire pas l’oracle séparé des frames figées ; les attentes résiduelles de l’abattage restent des échecs conservés, distincts d’un contrôle qui attendait artificiellement une frame entière supplémentaire.
