...
Tir/mêlée, adversaires, couvert, formation par glisser, orientation d’attente imposée, feu à volonté, personnalité et réactions de combat absents. Ordres d’équipement/soins civils pendant mobilisation encore refusés explicitement ; démobiliser permet ces actions. La boucle tactique est donc partielle, même si son déplacement est physique et persistant. Les modules `drafting-rules`, `drafting-destinations`, `drafting`, `drafting-save` séparent données, choix spatial, exécution et validation ; les prochains tirs doivent réutiliser ces frontières.
