# Noyau de simulation — schéma 6, besoins et matériaux
...
Le gameplay utilise encore un **Dijkstra CPU sur huit voisins**. Son champ d'accessibilité est partagé entre les candidats d'un colon, puis seul le chemin choisi est conservé. Les nouvelles tâches et dépôts réveillent les planificateurs ; sans changement, la cadence de réexamen habituelle est de 20 ticks.

La grille de blocage est construite au plus une fois par tick, lors du premier besoin de navigation ; aucun cache entre ticks ne masque les changements de terrain ou de commandes. V6 utilise une file de Dial et des voisins pondérés sans tableau par cellule visitée. La [mesure historique des grandes cartes](map-scale.md#mesure-du-noyau-cpu) concerne le BFS antérieur ; les [contrats et mesures V6](spatial-motion-storage.md) décrivent le moteur actuel.

Un tick partage deux budgets : **huit recherches pondérées** et **32 768 couples source/destination examinés**. Le produit piles × destinations n'est pas matérialisé dans un immense tableau de candidats. Des index locaux regroupent quantités et réservations ; une fenêtre tournante utilise `logisticsCursor`, persistant dans la sauvegarde. Lorsque tous les couples ne tiennent pas dans la fenêtre, le classement ne prétend pas avoir trouvé le meilleur transport global en une seule visite. Les scans d'entités, copies, besoins et autres travaux subsistent : ces bornes ne garantissent pas une durée de tick constante.
