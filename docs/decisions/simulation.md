...
14 septembre 2026, schéma 13 inchangé. Les mesures montrent que les recherches par groupes de l'ADR-024 calculaient beaucoup de distances inutiles avant le classement des tâches. Le planner distingue maintenant l'existence d'un accès des routes précises. Un parcours cardinal progressif et un Dijkstra pondéré reprenable partagent un instantané d'obstacles/occupations privé à la décision. La preuve d'équivalence des composantes dépend des deux côtés libres pour une diagonale ; elle doit être réexaminée si les règles de franchissement changent. [Contrat, sources et limites](../development/spatial-motion-storage.md#accès-aux-candidats-et-routes-précises).

La recherche pondérée conserve sa file et termine chaque couche de coût égal, pour garder les parents et départages du parcours complet. Les types séparent accessibilité et distances finalisées. Les besoins conservent leurs recherches ciblées ; les priorités, quantités, engagements, trajets et budgets ne changent pas. `planning-goals.ts`, devenu inutile, est retiré. Aucun état transitoire ne rejoint la sauvegarde ou le rendu.

Une première version parcourait immédiatement toute la composante : gain sous forte charge, mais travail superflu dans les petits camps. Le parcours à la demande est retenu après comparaison. Trente états complets sont byte-identiques à la version précédente, y compris lors d'une faim groupée ; oracle sur 120 cartes, pilote de plusieurs jours et worker réel complètent cette preuve. Les compteurs distinguent expansions d'accès et pondérées. Les cibles déconnectées et les collisions trop strictes restent ouvertes ; les buffers privés et leurs allocations devront être reprofilés si la population ou la topologie évolue.

## ADR-030 — Passage civil et réservations de service

14 septembre 2026, V14. La [relecture Core](../research/civil-traffic-reference.md) confirme que traiter tous les colons comme des obstacles rigides était une divergence de gameplay. Le profil civil utilise un ensemble de bloqueurs vide commun au planner et au suivi. `travel.ts` extrait le suivi auparavant dans `engine.ts`, conserve temps d'arête et contrôle des coins statiques, et retire les replis qui poussaient les inactifs. Les APIs exposent des ensembles en lecture seule ; aucun cache spatial supplémentaire ou moteur physique.
