...
**Précision de méthode demandée par l'utilisateur le 20 septembre : les sauvegardes de RimWorld sont une référence parmi plusieurs.** Elles documentent une trajectoire particulière, avec sa version, ses réglages, son site, ses choix de joueur et ses événements. Elles ne définissent ni la progression obligatoire de Lisière, ni un ordre universel des constructions, ni la date attendue d'une recherche ou d'une récolte. Une règle générale exige une confrontation aux définitions, aux conditions du moteur, au corpus et aux sources indépendantes disponibles ; un fait seulement observé reste présenté comme tel. Reproduire une sauvegarde ou achever son équipement ne doit pas devenir l'objectif implicite du développement. Les fronts compatibles du jeu continuent d'avancer ensemble selon ROADMAP, avec des critères propres à leurs boucles.
...
L'horizon d'un pilote découle des contrats qu'il doit éprouver, pas de la durée des parties de référence ni d'un nombre de jours imposé à chaque livraison. Traverser un cycle saisonnier était pertinent pour V87 afin de croiser froid, cultures, chauffage et réserves ; cela ne rend pas une année de colonie nécessaire pour les lots suivants. Les checkpoints acquis permettent de repartir du contexte utile, et une campagne commune peut valider plusieurs fronts sans reproduire la trajectoire d'un joueur de RimWorld. La [stratégie de validation](../development/testing.md) encadre la mesure des coûts et la réduction des répétitions sans changer les règles simulées.
