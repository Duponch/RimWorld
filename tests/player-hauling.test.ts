...
test('waiting hauling survives migration/replay, releases vanished sources, rejects corrupt quantities and legacy extensions atomically',()=>{
  ...
  const old=JSON.parse(serializeWorld(camp()));old.schemaVersion=17;delete old.deconstructed;delete old.packed;const migrated=deserializeWorld(JSON.stringify(old));expect(migrated).toEqual({...old,schemaVersion:26,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}});
