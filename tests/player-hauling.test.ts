...
test('waiting hauling survives migration/replay, releases vanished sources, rejects corrupt quantities and legacy extensions atomically',()=>{
  ...
  const old=JSON.parse(serializeWorld(camp()));old.schemaVersion=17;const migrated=deserializeWorld(JSON.stringify(old));expect(migrated).toEqual({...old,schemaVersion:20});
