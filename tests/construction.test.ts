...
function camp(count=1) {
  ...
  w.pawns.forEach((p,i)=>{p.x=8+i;p.z=10;p.hunger=100;p.rest=100;p.schedule.fill('anything');p.priorities={warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:1,haul:0,gather:0,grow:0,cook:0};});refreshStock(w);return w;
}
...
test('plant clearing respects a rotated footprint, saves mid-cut, and transport-only actors can supply but cannot finish a frame',()=>{
  ...
  const shipping=camp();shipping.pawns[0]!.priorities={warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:0,haul:1,gather:0,grow:0,cook:0};addGroundMaterial(shipping,'wood',45,{x:7,z:10},'wood');
...
test('plans and frames remain traversable with calibrated edge delay, completion protects active corners, and V15 migration is strict',()=>{
  ...
  Object.assign(passer,{x:10,z:10,priorities: {warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:0,haul:0,gather:0,grow:0,cook:0}});startTravel(w,passer,{x:11,z:11});
...
test('replacing storage with a blueprint releases active and queued deliveries atomically, preserves other zone cells, and migrates old furniture contents',()=>{
  for(const kind of ['wall','campfire','stool'] as const) {
    const w=camp(2),p=w.pawns[0]!;for(const q of w.pawns)q.priorities={warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:0,haul:0,gather:0,grow:0,cook:0};p.priorities.haul=1;
