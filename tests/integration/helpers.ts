...
export async function tool(page: Page, name: 'crafting-spot'|'home'|'remove-home'|'wood-generator'|'standing-lamp'|'passive-cooler'|'build-roof'|'remove-roof'|'ignore-roof'|'door' | 'stonecutter' | 'mine' | 'haul-chunks' | 'uninstall' | 'deconstruct' | 'select' | 'chop' | 'harvest' | 'cut' | 'cancel' | 'wall' | 'bed' | 'table' | 'horseshoes' | 'stool' | 'campfire' | 'stockpile' | 'remove-stockpile' | 'growing' | 'remove-growing') {
  await panel(page, 'architect');
  const category = name === 'wood-generator' ? 'power' : name === 'standing-lamp' ? 'furniture' : name === 'stonecutter' || name === 'crafting-spot' ? 'production' : name === 'horseshoes' ? 'recreation' : name === 'campfire' || name === 'passive-cooler' ? 'temperature' : name === 'door' || name === 'wall' ? 'structure' : name === 'bed' || name === 'table' || name === 'stool' ? 'furniture' : name==='home'||name==='remove-home'||name === 'build-roof' || name === 'remove-roof' || name === 'ignore-roof' || name === 'stockpile' || name === 'remove-stockpile' || name === 'growing' || name === 'remove-growing' ? 'zones' : 'orders';
