...
test('partie de trois jours : un joueur équipe son camp et entretient ses stocks par la vraie interface', async ({playwright},testInfo)=>{
  ...
  try {
    ...
    for(let hour=0;hour<=72;hour+=4) {
      ...
      if(hour===72) {
        expect(summary.mining.cells,context).toBe(6);expect(summary.mining.steelStored,context).toBe(80);expect(summary.mining.stored,context).toBe(summary.mining.chunks);
