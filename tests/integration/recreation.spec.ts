...
test('loisirs par interface : migration, piquet construit, horaire, activité physique, inspection et reprise',async({playwright},testInfo)=>{
  ...
  try {
    ...
    const old=JSON.parse(serializeWorld(fixture));old.schemaVersion=14;for(const pawn of old.pawns)delete pawn.orders;old.pawns.forEach((p:any)=>delete p.recreation);
    ...
    await expect.poll(async()=>(await world(page)).schemaVersion).toBe(18);expect((await world(page)).pawns.map(p=>p.recreation.level)).toEqual([55,55,55]);
