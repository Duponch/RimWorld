...
test('Affectations : régime partagé, copie, refus de suppression, faim, migration et commandes acquittées',async({playwright},testInfo)=>{
  ...
  try {
    ...
    fixture.pawns.forEach((p,i)=>{p.x=12+i*3;p.z=13;p.hunger=19;p.rest=100;p.priorities={craft:2,mine:2,gather:0,build:0,haul:0,grow:0,cook:0};});
    ...
    const old=withoutFoodPolicies(JSON.parse(serializeWorld(fed)));old.schemaVersion=12;for(const a of old.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete old.deconstructed;delete old.packed;
