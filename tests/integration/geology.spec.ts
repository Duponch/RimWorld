...
test('inspecter les cinq roches, sauvegarder leurs identités et reprendre une carte historique', async ({ playwright }, testInfo) => {
  ...
  try {
    ...
    for (const c of cells) {
      ...
      await expect(page.locator('#cell-description')).toContainText('Minage, fragments et sol rocheux');
    }
    ...
    const old = JSON.parse(serializeWorld(fixture)); old.schemaVersion = 26;
    ...
    await panel(page, 'menu'); await page.locator('#load').click(); await expect.poll(async () => (await world(page)).schemaVersion).toBe(27);
