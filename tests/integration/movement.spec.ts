...
test('civil crossing in the real worker: shared cell, save/reload, three exclusive beds',async({playwright},testInfo)=>{
  ...
  try {
    const fixture=civilCrossingFixture(),old=JSON.parse(serializeWorld(fixture));old.schemaVersion=13;delete old.deconstructed;delete old.packed;for(const pawn of old.pawns){delete pawn.orders;delete pawn.recreation;}
