...
`;
test('roof accident reaches health inspection at its visible tick; fallen carrier remains prone across pause and reload',async({playwright})=>{
  ...
  try{
    ...
    await page.locator(`[data-pawn="${initial.pawns[0]!.id}"]`).click();await expect(page.locator('#health-inspection')).toContainText('Décédé');await expect(page.locator('[data-health="injuries"]')).not.toHaveText('');await expect(page.locator('#fps-counter')).toBeVisible();await page.screenshot({path:'artifacts/health-roof-v45.png'});
    ...
    await panel(page,'menu');await page.locator('#save').click();await page.locator('#load').click();await expectWorld(page,stopped);await page.keyboard.press('Escape');await page.locator(`[data-pawn="${p.id}"]`).click();await expect(page.locator('#fps-counter')).toHaveText(/\d+ FPS/);await page.screenshot({path:'artifacts/health-downed-v45.png'});
    ...
    writeFileSync('artifacts/health-ui-v45.json',JSON.stringify({date:new Date().toISOString(),backend:'native WebGPU',accidentTick:accident.tick,firstVisibleInjury:transition,fallenPoseObservations:poses.length,stopped:stopped.pawns[0],errors},null,2)+'\n');
