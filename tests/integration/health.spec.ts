...
test('roof accident and prone cargo remain synchronized; loaded Gunshot wounds receive physical care and survive replay',async({playwright})=>{
  ...
  try{
    ...
    await page.locator(`[data-pawn="${initial.pawns[0]!.id}"]`).click();await pawnTab(page,'health');await expect(page.locator('#health-inspection')).toContainText('Décédé');await expect(page.locator('[data-health="injuries"]')).not.toHaveText('');await expect(page.locator('#fps-counter')).toBeVisible();await page.screenshot({path:`artifacts/health-roof-${proofVersion}.png`});
    ...
    await page.locator(`[data-pawn="${patient.id}"]`).click();await pawnTab(page,'health');await expect(page.locator('[data-health="injuries"]')).toContainText('Blessure par balle');await expect(page.locator('#fps-counter')).toBeVisible();
