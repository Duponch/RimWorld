...
test('Horaires : peindre, annuler, clavier, copier, reprendre et réveiller physiquement par le worker', async ({playwright}, testInfo) => {
  ...
  try {
    ...
    fixture.pawns.forEach((p,i)=>{p.x=13+i*3;p.z=13;p.rest=60;p.priorities={mine:2,gather:0,build:0,haul:0,grow:0,cook:0};fixture.structures.push({id:fixture.nextId++,kind:'bed',x:p.x,z:p.z,orientation:0,footprint:'standard'});p.bedId=fixture.structures.at(-1)!.id;});
    ...
    const old=withoutPostV11Fields(JSON.parse(serializeWorld(awake)));old.schemaVersion=11;for(const a of old.pawns)delete a.priorities.mine;delete old.deconstructed;delete old.packed;
