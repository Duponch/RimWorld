...
test('player enables treatment, orders at bedside, saves mid-care and observes actual GPU work and wound quality',async({playwright})=>{
  ...
  try{
    ...
    await page.locator(`[data-pawn="${patient.id}"]`).click();await page.locator('#medical-policy').uncheck();await expect.poll(async()=>(await world(page)).pawns[1]!.careDisabled).toBe(true);
    await page.locator('#medical-policy').check();await expect.poll(async()=>(await world(page)).pawns[1]!.careDisabled).toBeUndefined();
    ...
    await page.screenshot({path:'artifacts/care-bedside-v47.png'});await panel(page,'menu');await page.locator('#save').click();await page.locator('#load').click();await expectWorld(page,during);await page.keyboard.press('Escape');
    ...
    await page.locator(`[data-pawn="${patient.id}"]`).click();await expect(page.locator('#health-inspection')).toContainText('qualité');await expect(page.locator('#fps-counter')).toHaveText(/\d+ FPS/);await page.screenshot({path:'artifacts/care-treated-v47.png'});
