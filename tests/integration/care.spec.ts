...
test('player enables treatment, orders at bedside, saves mid-care and observes actual GPU work and wound quality',async({playwright})=>{
  ...
  try{
    ...
    await page.locator(`[data-pawn="${patient.id}"]`).click();await page.locator('#medical-policy').selectOption('none');await expect.poll(async()=>(await world(page)).pawns[1]!.medicalCare).toBe('none');
    ...
    await page.locator(`[data-pawn="${patient.id}"]`).click();await expect(page.locator('#health-inspection')).toContainText('qualité');await expect(page.locator('#fps-counter')).toHaveText(/\d+ FPS/);await page.screenshot({path:process.env.CARE_TREATED_IMAGE??'artifacts/care-treated-v47.png'});
