...
const index=new Map(this.world.pawns.map((p,i)=>[p.id,i]));this.world.pawns.forEach((p,i)=>{if(${care}&&p.tend?.phase==='tend'){b.carriedFrames++;const j=index.get(p.tend.patientId);if(m.getZ(j)!==1||m.getY(i)!==1||Math.abs(p.x-this.world.pawns[j].x)+Math.abs(p.z-this.world.pawns[j].z)!==1)b.invalidPoses++;}if(!${care}&&p.rescue?.phase==='carry'){b.carriedFrames++;const j=index.get(p.rescue.patientId);if(m.getZ(j)!==6||m.getX(j)!==0||m.getY(j)!==0||f.getX(i)!==f.getX(j)||f.getZ(i)!==f.getZ(j)||f.getW(i)!==f.getW(j)||t.getX(i)!==t.getX(j)||t.getZ(i)!==t.getZ(j)||clock.getX(i)!==clock.getX(j)||clock.getY(i)!==clock.getY(j))b.invalidPoses++;}});
}return result;};
`;
...
test('native medical load: 2/30/100 actors, exact carried poses and stable pipelines on a 250² map',async({playwright})=>{
  ...
  try{
    for(const pairs of [1,15,50]){
      ...
      const initial=(care?careCamp:rescueCamp)(pairs,250);for(const p of [...initial.pawns,...initial.structures]){p.x+=100;p.z+=100;}
      ...
      await page.waitForFunction(n=>{const b=(window as any).__miningBench;return (n.care?b.view.world.pawns.reduce((sum:number,p:any)=>sum+p.skills.medicine.xp,0)>=n.pairs*175000:b.view.world.pawns.filter((p:any)=>p.state==='downed'&&p.need?.kind==='sleep'&&p.need.bedId!==null).length===n.pairs);},{pairs,care},{timeout:45000,polling:100});
    ...
    writeFileSync(care?'artifacts/care-native-v47.json':'artifacts/rescue-native-v46.json',JSON.stringify({date:new Date().toISOString(),cpu:cpus()[0]?.model,viewport:{width:1440,height:1000},protocol:(care?'Physical treatments and medical rest; carriedFrames counts active bedside observations; rescued counts fully treated patients. ':'Physical rescue. ')+'Native WebGPU, real worker at 6x, cleared 250², concurrent medical services and live physiology; 90 warmup frames. No claim about forests or full combat.',results},null,2)+'\n');
