...
const medicine=process.env.MEDICINE_LOAD==='1';
const care=medicine||process.env.CARE_LOAD==='1';
...
const g=this.pawns.pawnMesh.geometry,f=g.getAttribute('aFrom'),t=g.getAttribute('aTo'),clock=g.getAttribute('aTravel'),m=g.getAttribute('aMotion'),cargo=g.getAttribute('aCargo');
const index=new Map(this.world.pawns.map((p,i)=>[p.id,i]));this.world.pawns.forEach((p,i)=>{if(${medicine}&&p.tend?.medicine?.carryPileId&&cargo.getX(i)!==19)b.invalidPoses++;if(${urgent}&&p.tend?.phase==='tend'){b.carriedFrames++;if(p.tend.patientId!==p.id||m.getZ(i)!==0||m.getY(i)!==1)b.invalidPoses++;}if((${care}&&p.tend?.phase==='tend')||(${feeding}&&p.feed?.phase==='feed')){b.carriedFrames++;const j=index.get(p.feed?.patientId??p.tend.patientId);if(m.getZ(j)!==1||m.getY(i)!==1||Math.abs(p.x-this.world.pawns[j].x)+Math.abs(p.z-this.world.pawns[j].z)!==1)b.invalidPoses++;}if(!${urgent}&&!${care}&&!${feeding}&&p.rescue?.phase==='carry'){b.carriedFrames++;const j=index.get(p.rescue.patientId);if(m.getZ(j)!==6||m.getX(j)!==0||m.getY(j)!==0||f.getX(i)!==f.getX(j)||f.getZ(i)!==f.getZ(j)||f.getW(i)!==f.getW(j)||t.getX(i)!==t.getX(j)||t.getZ(i)!==t.getZ(j)||clock.getX(i)!==clock.getX(j)||clock.getY(i)!==clock.getY(j))b.invalidPoses++;}});
}return result;};
`;
...
test('native medical load: 2/30/100 actors, exact carried poses and stable pipelines on a 250² map',async({playwright})=>{
  ...
  try{
    for(const pairs of process.env.MEDICAL_PROFILE?[50]:[1,15,50]){
      ...
      const initial=urgent?urgentSelfCamp(pairs*2,250):(medicine?medicineCamp:feeding?feedingCamp:care?careCamp:rescueCamp)(pairs,250);for(const p of [...initial.pawns,...initial.structures]){p.x+=100;p.z+=100;}
      ...
      await page.waitForFunction(n=>{const b=(window as any).__miningBench;return (n.feeding?b.view.world.pawns.filter((p:any,i:number)=>i%2===1&&p.hunger>80).length===n.pairs:n.care||n.urgent?b.view.world.pawns.reduce((sum:number,p:any)=>sum+p.skills.medicine.xp,0)>=n.pairs*(n.urgent?2:1)*(n.medicine?122500:175000):b.view.world.pawns.filter((p:any)=>p.state==='downed'&&p.need?.kind==='sleep'&&p.need.bedId!==null).length===n.pairs);},{pairs,care,feeding,urgent,medicine},{timeout:45000,polling:100});
      await page.locator('[data-speed="0"]').click();
      if(profiler){const result=await profiler.send('Profiler.stop');writeFileSync('tmp/medical-main-profile.cpuprofile',JSON.stringify(result.profile));await profiler.detach();}
      ...
      const end=await world(page);expect(validateWorld(end)).toEqual([]);if(medicine)expect(end.piles.reduce((n,p)=>n+(p.kind==='medicine'?p.quantity:0),0)).toBe(pairs*3);expect(b.carriedFrames).toBeGreaterThan(pairs);expect(b.invalidPoses).toBe(0);expect(b.stableGeometry).toBe(true);expect(b.pipelines).toEqual([]);expect(errors).toEqual([]);
    ...
    writeFileSync(process.env.MEDICAL_LOAD_REPORT??(medicine?'artifacts/medicine-native-v51.json':urgent?'artifacts/urgent-care-native-v50.json':feeding?'artifacts/feeding-native-v48.json':care?'artifacts/care-native-v47.json':'artifacts/rescue-native-v46.json'),JSON.stringify({date:new Date().toISOString(),cpu:cpus()[0]?.model,viewport:{width:1440,height:1000},protocol:(medicine?'Physical medicine collection and bedside batch treatment; one dose per patient; cargo pose checked. ':urgent?'Urgent self-treatment followed by ordinary care; carriedFrames counts work observations; rescued counts fully treated adults. ':feeding?'Physical feeding; carriedFrames counts bedside observations; rescued counts fed patients. ':care?'Physical treatments and medical rest; carriedFrames counts active bedside observations; rescued counts fully treated patients. ':'Physical rescue. ')+'Native WebGPU, real worker at 6x, cleared 250², concurrent medical services and live physiology; 90 warmup frames. Scene CPU is within frame CPU; snapshot callback includes HUD scheduling, not complete IPC/decoding. No claim about forests or full combat.',results},null,2)+'\n');
