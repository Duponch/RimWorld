...
test('native fauna targeting, real injury/flight, continuous GPU travel and save at 1×/6×',async({playwright})=>{
  ...
  try {
    ...
    for(const speed of [1,6]){
      ...
      await panel(page,'wildlife');await page.screenshot({path:`artifacts/animal-combat-${speed}x-v77.png`});
    ...
    expect(errors).toEqual([]);writeFileSync('artifacts/animal-combat-ui-v77.json',JSON.stringify({date:new Date().toISOString(),reports,errors},null,2));
