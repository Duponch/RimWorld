...
const probe=`window.__miningBench={active:false,pipelines:[],frames:[],scenes:[],snapshots:[],previous:null,view:null,visible:0};
...
const shotFrame=ColonyRenderer.prototype.frame;ColonyRenderer.prototype.frame=function(now){const b=window.__miningBench;b.view=this;const start=performance.now(),r=shotFrame.call(this,now);if(b.active&&!this.preparing){b.frames.push({cpu:performance.now()-start,interval:b.previous===null?null:now-b.previous,calls:this.stats.drawCalls});b.previous=now;const g=this.projectiles.mesh.geometry,t=g.getAttribute('bulletTime');for(let i=0;i<g.instanceCount;i++)if(this.projectiles.tick.value>=t.getX(i)&&this.projectiles.tick.value<t.getY(i))b.visible++;}return r;};`;
...
test('native mixed shooting load: 3/30/100 actors, 250², worker/scene/frames and GPU capacity growth',async({playwright})=>{
  ...
  try {for(const count of [3,30,100]) {
    const page=await browser.newPage({baseURL:'http://127.0.0.1:5173',viewport:{width:1440,height:1000}}),errors=observeErrors(page),{world:initial,pairs}=shootingLoad(count);
    for(const [pawnId,targetId] of pairs)expect(applyCommand(initial,{type:'shoot',pawnIds:[pawnId],targetId}).ok).toBe(true);
    ...
    const b=await page.evaluate(()=>{const b=(window as any).__miningBench;b.active=false;const device=b.view.renderer.getContext().getConfiguration().device;return {frames:b.frames,scenes:b.scenes,snapshots:b.snapshots,pipelines:b.pipelines,visible:b.visible,capacity:b.view.projectiles.mesh.geometry.getAttribute('bulletTime').count,adapter:{vendor:device.adapterInfo.vendor,architecture:device.adapterInfo.architecture}};});
    expect(b.visible).toBeGreaterThan(0);expect(b.pipelines).toEqual([]);expect(errors).toEqual([]);if(count===100)expect(b.capacity).toBeGreaterThan(32);
    results.push({actors:count,pairs:pairs.length,adapter:b.adapter,framesMs:stats(b.frames.flatMap((f:any)=>f.interval===null?[]:[f.interval])),frameCpuMs:stats(b.frames.map((f:any)=>f.cpu)),sceneMs:stats(b.scenes),snapshotCallbackMs:stats(b.snapshots),drawCalls:stats(b.frames.map((f:any)=>f.calls)),visibleFlightSamples:b.visible,capacity:b.capacity,gunshotPatients:end.pawns.filter(p=>p.health?.injuries.some(i=>i.kind==='gunshot')).length,pipelines:b.pipelines,errors});await page.close();
  }writeFileSync('artifacts/shooting-native-v56.json',JSON.stringify({date:new Date().toISOString(),cpu:cpus()[0]?.model,viewport:{width:1440,height:1000},protocol:'Native WebGPU, real worker at 6× for 240 ticks, generated forest 250² with cleared shooting lanes; one third shoots, one third targets, remainder mines/chops/hauls. Actual shot commands accepted before fixture load; health, XP and all decisions evolve in worker. 90 warmup frames before measurement. Scene time is included in frame time; callback excludes IPC decoding. No claim of perfect frame pacing.',results},null,2)+'\n');
