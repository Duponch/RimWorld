...
test('kitchen role, powered idle heat, work heat, fractional last fuel, outage and strict version boundaries',()=>{
  ...
  for(const kind of ['fueled-stove','electric-stove','butcher-table'] as const){const future=withoutFoodCrops(camp());station(future,kind);const bad=JSON.parse(serializeWorld(future));bad.schemaVersion=83;expect(()=>deserializeWorld(JSON.stringify(bad))).toThrow();}
  const old=withoutFoodCrops(camp()),fire:Structure={id:old.nextId++,kind:'campfire',x:10,z:10,orientation:0,footprint:'standard',fuel:{ticks:12000,burned:0,autoRefuel:true},bills:[newCookingBill(old.nextId++)]};old.structures.push(fire);delete fire.bills![0]!.filters.potato;delete fire.bills![0]!.filters.corn;const legacy=JSON.parse(serializeWorld(old));legacy.schemaVersion=83;const migrated=deserializeWorld(JSON.stringify(legacy));expect(migrated.structures[0]!.bills![0]!.filters).toEqual(fire.bills![0]!.filters);legacy.structures[0].bills[0].filters.corn=false;expect(()=>deserializeWorld(JSON.stringify(legacy))).toThrow();
});
