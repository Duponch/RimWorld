...
test('self-treatment roundtrips snapshots and rejects corrupted or premature V48 data',()=>{
  const old=selfTendingCamp();old.schemaVersion=48 as World['schemaVersion'];expect(deserializeWorld(JSON.stringify(old))).toEqual({...old,schemaVersion:SCHEMA_VERSION});
