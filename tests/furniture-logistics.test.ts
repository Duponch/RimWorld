...
test('whole parcels reserve exclusive sources and floor slots, queue through player commands and resume without copying their identities',()=>{
  const w=deconstructionCamp(2),p=w.pawns[0]!,other=w.pawns[1]!;p.priorities={ doctor:0,craft:2,mine:2,build:0,haul:1,gather:0,grow:0,cook:0};other.priorities.build=0;
