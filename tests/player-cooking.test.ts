...
function camp():World {
  ...
  w.pawns.forEach((p,i)=>{Object.assign(p,{x:8+i,z:10,hunger:100,rest:100});p.schedule.fill('anything');p.priorities={hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:0,gather:0,grow:0,cook:0};});refreshStock(w);return w;
}
...
test('queued cooking reserves mixed ingredients and staging, follows bill order, survives every phase with disabled work and produces exactly one meal',()=>{
  const w=camp(),[p,q]=w.pawns,s=fire(w),job=woodJob(w);p!.priorities={hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:0,gather:1,grow:0,cook:1};
...
test('manual recipe respects suspension/radius, refuels first without hauling, rejects corrupt queues and preserves ingredients when recipes or food disappear',()=>{
  const w=camp(),p=w.pawns[0]!,s=fire(w);w.pawns=w.pawns.slice(0,1);p.priorities={hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:0,gather:1,grow:0,cook:1};
  ...
  const legacy=JSON.parse(serializeWorld(camp()));(legacy.schemaVersion=19,withoutPawnSkills(legacy));for(const a of legacy.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete legacy.deconstructed;delete legacy.packed;expect(deserializeWorld(JSON.stringify(legacy))).toEqual(withMigratedSkills({...legacy,pawns:legacy.pawns.map((p:any)=>({...p,priorities: {hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,...p.priorities,mine:2}})),schemaVersion:SCHEMA_VERSION,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}}));
});
...
test('forced sowing clearance uses growing assignment, survives regenerated intents and cancels a carried stack conservatively when the zone changes',()=>{
  const w=camp(),p=w.pawns[0]!;w.pawns=w.pawns.slice(0,1);p.priorities={hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:0,gather:1,grow:1,cook:0};
