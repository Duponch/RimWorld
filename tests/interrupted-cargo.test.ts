...
test('schema 43 is checked before migration; passive ownership cannot hide illegal work, movement, duplicate objects or missing cargo',()=>{
  const initial=exhaustedCarrier(),legacy=JSON.parse(serializeWorld(initial));(legacy.schemaVersion=43,withoutResearch(legacy));withoutMedicalWork(legacy);
  ...
  expect(deserializeWorld(JSON.stringify(legacy))).toEqual(withMigratedResearch(expected));
