...
test('priority decisions share budgets, disappear on lost access or timeout without cancelling active work, and validate strict V22 migration',()=>{
  ...
  const migrated=deserializeWorld(JSON.stringify(raw));expect(migrated).toEqual({...raw,pawns:raw.pawns.map((p:any)=>({...p,priorities:{craft:2,...p.priorities,mine:2}})),schemaVersion:33,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}});expect(migrated.pawns[0]!.priorityWork).toBeUndefined();
});
