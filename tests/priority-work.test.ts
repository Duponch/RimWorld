...
function camp():World {
  ...
  const p=w.pawns[0]!;Object.assign(p,{x:8,z:10,hunger:100,rest:100});p.schedule.fill('anything');p.priorities={firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:1,haul:0,grow:0,gather:0,cook:1};refreshStock(w);return w;
}
...
test('priority decisions share budgets, disappear on lost access or timeout without cancelling active work, and validate strict V22 migration',()=>{
  ...
  const migrated=deserializeWorld(JSON.stringify(raw));expect(migrated).toEqual(withMigratedSkills({...raw,pawns:raw.pawns.map((p:any)=>({...p,priorities: {firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,...p.priorities,mine:2}})),schemaVersion:SCHEMA_VERSION,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}}));expect(migrated.pawns[0]!.priorityWork).toBeUndefined();
});
