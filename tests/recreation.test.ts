...
test('skygazing requires arrival; unavailable or boring activities give no joy; V14 migration preserves existing state',()=>{
  ...
  const old=fixture();old.pawns.forEach(p=>p.schedule.fill('anything'));const raw=JSON.parse(serializeWorld(old));raw.schemaVersion=14;raw.pawns.forEach((p:any)=>delete p.recreation);
