...
function camp():World {
  ...
  Object.assign(w.pawns[0]!,{x:2,z:2,hunger:20,rest:100,priorities:{craft:2,mine:2,gather:0,build:0,haul:0,grow:0,cook:0}});refreshStock(w);return w;
}
...
test('régimes partagés : commandes atomiques, copie indépendante, limites et migration V12 sans réécrire les états',()=>{
  ...
  const restored=deserializeWorld(JSON.stringify(old));expect(restored.schemaVersion).toBe(34);expect(restored.pawns.every(p=>p.foodPolicyId===1)).toBe(true);
