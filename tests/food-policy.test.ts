...
test('régimes partagés : commandes atomiques, copie indépendante, limites et migration V12 sans réécrire les états',()=>{
  ...
  const old=withoutFoodPolicies(JSON.parse(full));old.schemaVersion=12;
  const restored=deserializeWorld(JSON.stringify(old));expect(restored.schemaVersion).toBe(20);expect(restored.pawns.every(p=>p.foodPolicyId===1)).toBe(true);
  const stripped=withoutFoodPolicies(JSON.parse(serializeWorld(restored)));stripped.schemaVersion=12;expect(stripped).toEqual(old);
