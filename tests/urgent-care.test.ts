...
test('urgent state migrates strictly, survives snapshots, and stops on policy/incapacity without premature results',()=>{
  const old=urgentSelfCamp();old.schemaVersion=49 as World['schemaVersion'];const copy=deserializeWorld(JSON.stringify(old));expect(copy).toEqual({...old,schemaVersion:51});
