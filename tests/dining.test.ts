...
function fixture(count = 1, size = 64): World {
  ...
  w.pawns.forEach((p, i) => { p.x = 2; p.z = 2 + i * 2; p.hunger = 20; p.rest = 100; p.comfort = 10; p.priorities = {mine:2, gather: 0, build: 0, haul: 0, grow: 0 , cook: 0 }; });
...
test('wood must be delivered before furniture construction; comfort approaches a ceiling and meal memory expires without stacking', () => {
  ...
  p.hunger = 100; p.priorities = {mine:2, gather: 0, build: 1, haul: 1, grow: 0 , cook: 0 };
