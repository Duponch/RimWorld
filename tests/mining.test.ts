...
test('steel veins preserve topology, extraction commits 40 units, transport splits and merges real stacks, V28 is validated before migration',()=>{
  ...
  const migrated=deserializeWorld(JSON.stringify(old));expect(migrated.schemaVersion).toBe(37);expect(migrated.tiles).toEqual(old.tiles);expect(migrated.stockpiles).toEqual(old.stockpiles);
...
test('rough terrain and chunks share weighted and physical travel, strict V27 migration rejects future fields and resumes active edges',()=>{
  ...
  const migrated=deserializeWorld(JSON.stringify(old));expect(migrated.schemaVersion).toBe(37);expect(migrated.pawns[0]!.priorities.mine).toBe(2);expect(migrated.tiles).toEqual(old.tiles);
