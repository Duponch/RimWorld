...
test('steel veins preserve topology, extraction commits 40 units, transport splits and merges real stacks, V28 is validated before migration',()=>{
  ...
  const old=JSON.parse(serializeWorld(miningCamp()));(old.schemaVersion=28,withoutPawnSkills(old));for(const a of old.pawns)delete a.priorities.craft;old.tiles[i]={terrain:'rock',stone:'granite',miningDamage:80};
...
test('rough terrain and chunks share weighted and physical travel, strict V27 migration rejects future fields and resumes active edges',()=>{
  ...
  const old=JSON.parse(serializeWorld(miningCamp()));(old.schemaVersion=27,withoutPawnSkills(old));for(const a of old.pawns){delete a.priorities.mine;delete a.priorities.craft;}
