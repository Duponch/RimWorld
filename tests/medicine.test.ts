...
test('V50 migration rejects future medicine state, carries exact snapshots, and a hundred adults retain the material balance',()=>{
  const old=selfTendingCamp();for(const p of old.pawns)delete p.medicalCare;old.schemaVersion=50 as World['schemaVersion'];expect(deserializeWorld(JSON.stringify(old))).toEqual({...old,schemaVersion:52});
