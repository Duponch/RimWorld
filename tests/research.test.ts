...
test('cotton colony mines and builds its research chain, researches from zero, makes and wears the unlocked shirt',()=>{
  ...
  writeFileSync('artifacts/research-checkpoint-v73.json',serializeWorld(w));
  ...
  writeFileSync('artifacts/research-colony-v73.json',JSON.stringify({start,end:w.tick,research:w.research,skills:p.skills,shirt,slept,ate,buildings:w.structures.map(s=>s.kind),stock:w.stock},null,2));
},30000);
