...
function field(size=16):World {
  ...
  w.pawns=w.pawns.slice(0,1);Object.assign(w.pawns[0]!,{x:2,z:2,hunger:100,rest:100,priorities:{gather:0,build:0,haul:0,grow:0,cook:0}});
...
test('four, fourteen and forty days; stable snapshots, expiry batches, migration and corrupt-age rejection',()=>{
  ...
  const old=withoutPostV10Fields(JSON.parse(saved));old.schemaVersion=10;delete old.deconstructed;delete old.packed;
