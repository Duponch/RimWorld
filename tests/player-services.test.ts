...
test('forced refuel bypasses automation, reserves its station while queued, travels and works, persists every phase and conserves fuel on interruption',()=>{
  ...
  const legacy=JSON.parse(serializeWorld(camp()));legacy.schemaVersion=18;delete legacy.deconstructed;expect(deserializeWorld(JSON.stringify(legacy))).toEqual({...legacy,schemaVersion:24,deconstructed:{count:0,lostWood:0,fuelTicks:0}});
});
...
test('forced plant and pile clearing respects rotated footprints, queue cancellation, physical output, parent lifetime and construction assignment without ordinary hauling',()=>{
  ...
  const raw=JSON.parse(serializeWorld(w));raw.schemaVersion=18;delete raw.deconstructed;expect(()=>deserializeWorld(JSON.stringify(raw))).toThrow(/version 18/);
