...
function camp():World {
  ...
  w.pawns.forEach((p,i)=>{p.x=8;p.z=10+i;p.hunger=100;p.rest=100;p.schedule.fill('anything');p.priorities={research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:1,build:0,grow:0,gather:0,cook:0};});refreshStock(w);return w;
}
...
test('forced refuel bypasses automation, reserves its station while queued, travels and works, persists every phase and conserves fuel on interruption',()=>{
  ...
  q!.priorities={research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:0,gather:0,grow:0,cook:0};
  ...
  const legacy=JSON.parse(serializeWorld(camp()));(legacy.schemaVersion=18,withoutPawnSkills(legacy));for(const a of legacy.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete legacy.deconstructed;delete legacy.packed;expect(deserializeWorld(JSON.stringify(legacy))).toEqual(withMigratedSkills({...legacy,pawns:legacy.pawns.map((p:any)=>({...p,priorities: {research:0, patient:0,bedrest:0,doctor:0,craft:2,...p.priorities,mine:2}})),schemaVersion:SCHEMA_VERSION,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}}));
});
...
test('forced plant and pile clearing respects rotated footprints, queue cancellation, physical output, parent lifetime and construction assignment without ordinary hauling',()=>{
  const w=camp(),p=w.pawns[0]!;w.pawns=w.pawns.slice(0,1);p.priorities={research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2,haul:0,build:1,grow:0,gather:0,cook:0};
