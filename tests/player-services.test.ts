...
test('forced refuel bypasses automation, reserves its station while queued, travels and works, persists every phase and conserves fuel on interruption',()=>{
  ...
  q!.priorities={handle:0,clean:0,firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,art:0,craft:2,mine:2,haul:0,build:0,gather:0,grow:0,cook:0};
  ...
  const legacy=JSON.parse(serializeWorld(camp()));(legacy.schemaVersion=18,withoutPawnSkills(legacy));for(const a of legacy.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete legacy.deconstructed;delete legacy.packed;expect(deserializeWorld(JSON.stringify(legacy))).toEqual(withMigratedSkills({...legacy,pawns:legacy.pawns.map((p:any)=>({...p,priorities: {handle:0,clean:0,firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,...p.priorities,mine:2}})),schemaVersion:SCHEMA_VERSION,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}}));
});
