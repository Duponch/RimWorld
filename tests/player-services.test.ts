...
test('forced refuel bypasses automation, reserves its station while queued, travels and works, persists every phase and conserves fuel on interruption',()=>{
  ...
  const legacy=JSON.parse(serializeWorld(camp()));legacy.schemaVersion=18;delete legacy.deconstructed;delete legacy.packed;expect(deserializeWorld(JSON.stringify(legacy))).toEqual({...legacy,schemaVersion:27,packed:[],deconstructed:{count:0,lostWood:0,fuelTicks:0}});
});
