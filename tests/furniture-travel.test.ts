...
test('opposite loaded trips cross furniture, cancel in transit without teleport or loss, and migrate old edges strictly',()=>{
  ...
  const original=furnitureTrafficFixture(),raw=JSON.parse(serializeWorld(original));raw.schemaVersion=21;for(const a of raw.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete raw.deconstructed;delete raw.packed;
  expect(deserializeWorld(JSON.stringify(raw))).toEqual(original);
  const invalid=JSON.parse(serializeWorld(original));invalid.pawns[0].x=7;invalid.pawns[0].z=8;invalid.pawns[0].path=[];invalid.schemaVersion=21;for(const a of invalid.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete invalid.deconstructed;delete invalid.packed;
  ...
  for(const service of ['meal','ground-sleep','edge'] as const) {
    ...
    refreshStock(old);const raw=structuredClone(old) as any;raw.schemaVersion=21;for(const a of raw.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete raw.deconstructed;delete raw.packed;
