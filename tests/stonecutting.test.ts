...
test('ordres réservés, métier désactivé, annulation conservatrice, factures et meuble entier',()=>{
  ...
  const legacy=JSON.parse(serializeWorld(stonecuttingCamp()));((legacy.schemaVersion=31,withoutResearch(legacy)),withoutPawnSkills(legacy));for(const a of legacy.pawns)delete a.priorities.craft;for(const b of legacy.structures)delete b.bills;
