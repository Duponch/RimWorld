...
test('thermal food: freeze/thaw intervals, real owner transfers, mixing, threshold expiry and legacy continuation',()=>{
  ...
  const legacy=createWorld(1,32,32),raw=JSON.parse(serializeWorld(legacy));(raw.schemaVersion=37,withoutPawnSkills(raw));raw.tiles=raw.tiles.map((t:any)=>t.ore==='machinery'?(({ore:_ore,...rest})=>rest)(t):t);
  const migrated=deserializeWorld(JSON.stringify(raw));expect(migrated).toEqual(withMigratedSkills({...raw,schemaVersion:43}));
...
test('plant climate: integrated local history, query independence, roof changes, migration and deltas',()=>{
  ...
  const legacy=plantClimateFixture(3),raw=JSON.parse(serializeWorld(legacy));(raw.schemaVersion=38,withoutPawnSkills(raw));delete raw.resources[0].growthThermalFactor;raw.resources[0].growthTick=1000;
  const migrated=deserializeWorld(JSON.stringify(raw));expect(migrated).toEqual(withMigratedSkills({...raw,schemaVersion:43}));const oldGrowth=plantGrowth(migrated,migrated.resources[0]!);updatePlantTemperatures(migrated,reconcileTemperature(migrated));expect(plantGrowth(migrated,migrated.resources[0]!)).toBe(oldGrowth);
