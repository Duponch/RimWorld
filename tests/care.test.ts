...
test('strict V46 migration and relational rejects preserve continuing treatment by value',()=>{
  ...
  const old=careCamp();for(const p of old.pawns){delete (p.skills as Partial<typeof p.skills>).medicine;delete (p.priorities as Partial<typeof p.priorities>).patient;delete (p.priorities as Partial<typeof p.priorities>).bedrest;}(old.schemaVersion=46 as World['schemaVersion'],withoutResearch(old));withoutShootingSkills(old);
