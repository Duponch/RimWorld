...
test('strict V46 migration and relational rejects preserve continuing treatment by value',()=>{
  ...
  const upgraded=deserializeWorld(JSON.stringify(old));expect(upgraded.schemaVersion).toBe(51);expect(upgraded.pawns[0]!.skills.medicine).toEqual({level:8,xp:0,dailyXp:0,passion:0});expect(upgraded.pawns[0]!.priorities.patient).toBe(1);expect(upgraded.pawns[0]!.priorities.bedrest).toBe(3);
