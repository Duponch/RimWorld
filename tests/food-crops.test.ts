...
function field():World {
  ...
  p.priorities={hunt:0,research:0,patient:0,bedrest:0,doctor:0,gather:0,build:0,mine:0,grow:1,haul:2,cook:0,craft:0};refreshStock(w);return w;
}
...
test('V83 cannot hide future crop data; valid historical policies stay exact and four resident shapes never mutate the world',()=>{
  ...
  const old=JSON.parse(serializeWorld(w));old.schemaVersion=83;
