...
test('new encounter is reproducible, opt-in and separated from strict V57 migration',()=>{
  ...
  const legacy=structuredClone(createWorld()) as any;(legacy.schemaVersion=57,withoutResearch(legacy));legacy.piles=legacy.piles.filter((p:any)=>p.kind!=='apparel');const skills=legacy.pawns.map((p:any)=>structuredClone(p.skills));for(const p of legacy.pawns)delete p.skills.melee;const migrated=deserializeWorld(JSON.stringify(legacy));expect(migrated).toEqual(withMigratedResearch({...legacy,schemaVersion:SCHEMA_VERSION,pawns:legacy.pawns.map((p:any,i:number)=>({...p,skills:skills[i]}))}));
...
test.each([{mobile:false,sleeping:false},{mobile:true,sleeping:false},{mobile:false,sleeping:true}])('player encounter (%j) → downed ally → defense → rescue/medicine and one day of recovery',({mobile,sleeping})=>{
  ...
  for(let i=0;i<6200;i++){
    ...
    if(w.pawns.some(p=>p.rescue?.phase==='carry'&&p.rescue.patientId===patient.id)){carried=true;if(!copy)copy=deserializeWorld(serializeWorld(w));}
  ...
  expect(patient.state).not.toBe('dead');expect(w.pawns.some(p=>p.skills.medicine.xp>0)).toBe(true);
