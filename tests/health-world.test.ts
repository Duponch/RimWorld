...
test('strict V44 migration adds no injury; malformed records, clock, states and activity are rejected before replacement',()=>{
  const w=medicalCamp(),p=w.pawns[0]!;const old=structuredClone(w);old.schemaVersion=44 as typeof old.schemaVersion;withoutMedicalWork(old);const migrated=deserializeWorld(JSON.stringify(old));expect(migrated).toEqual({...old,schemaVersion:52,pawns:old.pawns.map(p=>({...p,skills:{...p.skills,medicine:{level:8,xp:0,dailyXp:0,passion:0}},priorities:{...p.priorities,doctor:1,patient:1,bedrest:3}}))});
