...
test('full floors and conflicting uses refuse atomically; migration and corrupt identity/carrier/target data remain strict',()=>{
 ...
 const legacy=JSON.parse(before);legacy.schemaVersion=24;delete legacy.packed;const migrated=deserializeWorld(JSON.stringify(legacy));expect(migrated.packed).toEqual([]);expect(migrated.structures).toEqual(JSON.parse(before).structures);expect(migrated.schemaVersion).toBe(26);
