...
test('full floors and conflicting uses refuse atomically; migration and corrupt identity/carrier/target data remain strict',()=>{
 ...
 const legacy=JSON.parse(before);legacy.schemaVersion=24;for(const a of legacy.pawns)delete a.priorities.mine;delete legacy.packed;const migrated=deserializeWorld(JSON.stringify(legacy));expect(migrated.packed).toEqual([]);expect(migrated.structures).toEqual(JSON.parse(before).structures);expect(migrated.schemaVersion).toBe(31);
