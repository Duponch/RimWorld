...
describe('seeded temperate valley generation', () => {
  test('regional Core geology is persistent, coherent, independent of work RNG and strictly migrated', () => {
    ...
    expect(migrated).toEqual(withMigratedSkills({...raw,pawns:raw.pawns.map((p:any)=>({...p,priorities: {hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,...p.priorities,mine:2}})),schemaVersion:SCHEMA_VERSION }));
  ...
  test('tutorial resources support a wall and stool even on tiny generated fixtures, including save during work', () => {
    ...
    const pawn = long.pawns[0]!; pawn.x = 0; pawn.z = 125; pawn.priorities = {hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:2, gather: 1, build: 0, haul: 0, grow: 0 , cook: 0 };
