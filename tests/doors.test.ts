...
test('a sleeping colonist crosses a real closed corridor door; navigation, solid corners, migration and corrupt saves agree',()=>{
  ...
  const legacy=deconstructionCamp();const old=JSON.parse(serializeWorld(legacy));old.schemaVersion=33;expect(deserializeWorld(JSON.stringify(old))).toEqual(legacy);
  const future=JSON.parse(saved);future.schemaVersion=33;expect(()=>deserializeWorld(JSON.stringify(future))).toThrow(/version 33/);
