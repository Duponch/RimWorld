...
test('ordinary player builds, welcomes, faces a naturally scheduled raid, demobilizes and maintains the same camp for five days',()=>{
  const w=createWorld(42,250,250);enableArrivals(w);enableRaids(w);
  ...
  for(let i=0;i<30000;i++){
    ...
    if(w.raids!.active&&!began){began=w.tick;writeFileSync('tmp/raid-camp-v68.json',serializeWorld(w));expect(w.structures.filter(s=>s.kind==='bed').length).toBeGreaterThanOrEqual(3);}
  ...
  const summary=colonySummary(w);writeFileSync('artifacts/raid-colony-v68.json',JSON.stringify({began,ended,injured,treated,postMeal,postWork,raid:w.raids,decisions,checkpoints,summary},null,2));
  ...
  expect(w.pawns.filter(isColonist)).toHaveLength(4);expect(w.pawns.filter(isColonist).every(p=>p.state!=='dead'&&p.state!=='downed'&&!p.draft)).toBe(true);
