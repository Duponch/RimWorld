...
test('civil crossing preserves beds, opposing cargo, every edge and exact continuation without pushing other actors',()=>{
  ...
  const old=JSON.parse(serializeWorld(w));old.schemaVersion=13;delete old.deconstructed;delete old.packed;for(const pawn of old.pawns)delete pawn.orders;
...
test('floor stacks enforce identity, reserved destination type, migration and atomic refusal when no drop fits',()=>{
  ...
  w.pawns[0]!.haul=null;
  const old=JSON.parse(serializeWorld(w));old.schemaVersion=5;delete old.deconstructed;delete old.packed;withoutPostV10Fields(old);for(const p of old.pawns){delete p.cooking;delete p.priorities.cook;}old.piles[1].owner={...old.piles[0].owner};
  ...
  const impossible=JSON.parse(JSON.stringify(old));impossible.schemaVersion=6;delete impossible.deconstructed;delete impossible.packed;expect(()=>deserializeWorld(JSON.stringify(impossible))).toThrow(/floor cell/);
