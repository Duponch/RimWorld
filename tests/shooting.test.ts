...
test('V55 migration is strict; current phase shape and skill cannot conceal invalid ownership or timing',()=>{
  ...
  const migrated=deserializeWorld(JSON.stringify(legacy));expect(migrated.schemaVersion).toBe(56);expect(migrated.pawns[0].skills.shooting).toEqual({level:8,xp:0,dailyXp:0,passion:0});
