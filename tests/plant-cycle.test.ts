...
test('growers clear mixed stacks without storage: reservations, physical trips, interruption, blocked exits and V8 continuation', () => {
  ...
  const old=JSON.parse(serializeWorld(world));old.schemaVersion=8;delete old.deconstructed;delete old.packed;withoutPostV10Fields(old);for(const p of old.pawns){delete p.cooking;delete p.priorities.cook;}
  const migrated=deserializeWorld(JSON.stringify(old));expect(migrated.schemaVersion).toBe(26);
  old.schemaVersion=10;delete old.deconstructed;delete old.packed;for(const p of old.pawns){p.cooking=null;p.priorities.cook=2;}expect(migrated).toEqual(deserializeWorld(JSON.stringify(old)));
});
...
test('growing policies, interrupted sowing, migration and resident crop slots preserve their contracts', () => {
  ...
  const oldSave=JSON.parse(JSON.stringify(old));oldSave.schemaVersion=7;delete oldSave.deconstructed;delete oldSave.packed;withoutPostV10Fields(oldSave);delete oldSave.growingZones;delete oldSave.growingCursor;delete oldSave.environment;
...
test('full floor, migration, snapshot immutability and resident fruit disappear/reappear without geometry rebuild',()=>{
  ...
  const old=fixture(),legacy=JSON.parse(serializeWorld(old));legacy.schemaVersion=6;delete legacy.deconstructed;delete legacy.packed;withoutPostV10Fields(legacy);for(const p of legacy.pawns){delete p.cooking;delete p.priorities.cook;}legacy.resources[0].amount=14;
