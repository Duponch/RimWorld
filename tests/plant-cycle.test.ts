...
test('growers clear mixed stacks without storage: reservations, physical trips, interruption, blocked exits and V8 continuation', () => {
  ...
  world.pawns=world.pawns.slice(0,2);world.pawns.forEach((p,i)=>Object.assign(p,{x:3,z:4+i*2,hunger:100,rest:100,priorities: {handle:0,clean:0,firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,art:0,craft:2,mine:2,grow:1,haul:0,gather:0,build:0, cook: 0 }}));
  ...
  sealed.pawns=sealed.pawns.slice(0,1);Object.assign(sealed.pawns[0]!,{x:2,z:2,hunger:100,rest:100,priorities: {handle:0,clean:0,firefight:0,warden:0,basic:3,hunt:0,research:0, patient:0,bedrest:0,doctor:0,art:0,craft:2,mine:2,grow:1,haul:0,gather:0,build:0, cook: 0 }});
...
test('growing policies, interrupted sowing, migration and resident crop slots preserve their contracts', () => {
  ...
  const oldSave=JSON.parse(JSON.stringify(old));(oldSave.schemaVersion=7,withoutPawnSkills(oldSave));for(const a of oldSave.pawns){delete a.priorities.mine;delete a.priorities.craft;}delete oldSave.deconstructed;delete oldSave.packed;withoutPostV10Fields(oldSave);delete oldSave.growingZones;delete oldSave.growingCursor;delete oldSave.environment;
