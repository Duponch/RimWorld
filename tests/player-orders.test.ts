...
export function orderCamp(count=2):World {
  ...
  w.pawns.forEach((p,i)=>{p.x=12+i*2;p.z=16;p.hunger=100;p.rest=100;p.schedule.fill('anything');p.priorities={build:1,haul:0,gather:1,grow:1,cook:0};});
...
test('Shift waits for ingestion, accepted orders survive priority zero, explicit cancellation and collapse release reservations',()=>{
  ...
  const disabled=deserializeWorld(serializeWorld(w));ticks(disabled,280);expect(disabled.jobs).toEqual([]);expect(disabled.stock.wood).toBe(24);
...
test('queue loses an access, clear-orders preserves designations, construction cannot skip delivery, V16 migration is strict',()=>{
  ...
  const raw=JSON.parse(serializeWorld(fresh));raw.schemaVersion=16;delete raw.deconstructed;delete raw.packed;for(const a of raw.pawns)delete a.orders;
  const migrated=deserializeWorld(JSON.stringify(raw));expect(migrated.schemaVersion).toBe(26);expect(migrated.pawns[0]!.orders).toEqual({active:null,queue:[]});
