...
test('shared doctors and sources cannot double reserve; malformed phases and V47 claims are rejected',()=>{
  ...
  replay(w,2);const old=feedingCamp();old.schemaVersion=47 as World['schemaVersion'];const migrated=deserializeWorld(JSON.stringify(old));expect(migrated).toEqual({...old,schemaVersion:53});
});
