...
export function miningCamp(count=1):World {
  ...
  w.pawns=Array.from({length:count},(_,i)=>({...structuredClone(original),id:w.nextId++,name:`Mineur ${i}`,x:10+i,z:12,hunger:100,rest:100,priorities: {hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:1,haul:2,gather:0,build:0,grow:0,cook:0}}));
  return w;
}
...
export function miningLoad(count:number,steel=false,machinery=false):World {
  ...
  const original=structuredClone(w.pawns[0]!);
  w.pawns=Array.from({length:count},(_,i)=>({...structuredClone(original),id:w.nextId++,name:`Mineur ${i}`,x:110+i%10*3,z:110+Math.floor(i/10)*3,hunger:100,rest:100,priorities: {hunt:0,research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:1,haul:2,gather:2,build:0,grow:0,cook:0}}));
