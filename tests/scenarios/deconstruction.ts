...
export function deconstructionCamp(count=1,size=32):World {
  ...
  w.pawns=Array.from({length:count},(_,i)=>({...structuredClone(original),id:w.nextId++,name:`Bâtisseur ${i+1}`,
    ...
    priorities: { patient:0,bedrest:0,doctor:0,craft:2,mine:2,build:1,haul:0,gather:0,grow:0,cook:0}}));
  for(const p of w.pawns)p.schedule.fill('anything');refreshStock(w);return w;
}
