...
const entries=JSON.parse(manifest).saves;assert.equal(entries.length,10);
...
try{
  ...
  for(const id of ['salles-v103','art-v104','economie-v105']){
    ...
    assert.equal(await front.locator('input[name="test-colony"]').count(),10);
    await page.screenshot({path:`artifacts/art-public-${id}-catalogue-v105.png`});
    ...
    if(id!=='economie-v105')await page.locator('#browse-saves').click();
    else{
      ...
      await page.screenshot({path:'artifacts/art-public-work-v105.png'});
    }
  }
  assert.equal(await page.evaluate(()=>typeof window.__lisiere),'undefined');
  assert.deepEqual(report.errors,[]);report.passed=true;
}finally{await browser.close();await writeFile('artifacts/economy-public-v105.json',JSON.stringify(report,null,2)+'\n');}
