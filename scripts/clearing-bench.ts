...
function fixture(count: number, obstructed: boolean) {
  ...
  w.pawns=Array.from({length:count},(_,i)=>({...structuredClone(template),id:w.nextId++,x:85+i%10*6,z:85+Math.floor(i/10)*8,hunger:100,rest:100,priorities:{grow:1,haul:0,build:0,gather:0, cook: 0 }}));
