...
import { medicineCamp } from '../tests/scenarios/medicine.ts';
...
const medicine=process.argv.includes('--medicine');
const care=medicine||process.argv.includes('--care');
...
for(const pairs of [1,15,50]){
  const w=self?(urgent?urgentSelfCamp:selfTendingCamp)(pairs*2,250):(medicine?medicineCamp:feeding?feedingCamp:care?careCamp:rescueCamp)(pairs,250),samples:number[]=[],copies:number[]=[];let carryTicks=0;
  ...
  const rescued=feeding?w.pawns.filter((p,i)=>i%2===1&&p.hunger>80).length:care||self?w.pawns.reduce((n,p)=>n+p.skills.medicine.xp,0)/(medicine?122500:175000):w.pawns.filter(p=>p.state==='downed'&&p.need?.kind==='sleep'&&p.need.bedId!==null).length;
  ...
  if(medicine&&w.piles.reduce((n,p)=>n+(p.kind==='medicine'?p.quantity:0),0)!==pairs*3)throw Error('Medicine balance differs');
...
const report={date:new Date().toISOString(),cpu:cpus()[0]?.model,node:process.version,os:`${platform()} ${release()}`,scope:(medicine?'Physical medicine pickup and multi-wound treatment; one industrial dose per patient. ':urgent?'Urgent self-treatment then ordinary reevaluation; carryTicks counts work ticks; rescued counts fully treated adults. ':self?'Ordinary self-treatment; carryTicks counts work ticks; rescued counts fully self-treated adults. ':feeding?'Physical feeding; carryTicks counts bedside ticks; rescued counts fed patients. ':care?'Physical treatments and medical rest; carryTicks counts active doctor ticks; rescued counts fully treated patients. ':'Physical rescues; carryTicks counts active carrying ticks. ')+'Cleared 250x250 map; concurrent reservations, continuing health/needs; cold and warm ticks included, clones separate',results};
writeFileSync(medicine?'artifacts/medicine-cpu-v51.json':urgent?'artifacts/urgent-care-cpu-v50.json':self?'artifacts/self-tending-cpu-v49.json':feeding?'artifacts/feeding-cpu-v48.json':care?'artifacts/care-cpu-v47.json':'artifacts/rescue-cpu-v46.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
