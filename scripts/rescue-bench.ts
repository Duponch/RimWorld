...
for(const pairs of [1,15,50]){
  const w=(feeding?feedingCamp:care?careCamp:rescueCamp)(pairs,250),samples:number[]=[],copies:number[]=[];let carryTicks=0;
  if(validateWorld(w).length)throw Error(validateWorld(w).join(';'));
  for(let t=0;t<(feeding||care?800:400);t++){
    const now=performance.now();stepWorld(w);samples.push(performance.now()-now);
    carryTicks+=w.pawns.filter(p=>feeding?p.feed?.phase==='feed':care?p.tend?.phase==='tend':p.rescue?.phase==='carry').length;
  ...
  const rescued=feeding?w.pawns.filter((p,i)=>i%2===1&&p.hunger>80).length:care?w.pawns.reduce((n,p)=>n+p.skills.medicine.xp,0)/175000:w.pawns.filter(p=>p.state==='downed'&&p.need?.kind==='sleep'&&p.need.bedId!==null).length;
...
const report={date:new Date().toISOString(),cpu:cpus()[0]?.model,node:process.version,os:`${platform()} ${release()}`,scope:(feeding?'Physical feeding; carryTicks counts bedside ticks; rescued counts fed patients. ':care?'Physical treatments and medical rest; carryTicks counts active doctor ticks; rescued counts fully treated patients. ':'Physical rescues; carryTicks counts active carrying ticks. ')+'Cleared 250x250 map; concurrent reservations, continuing health/needs; cold and warm ticks included, clones separate',results};
writeFileSync(feeding?'artifacts/feeding-cpu-v48.json':care?'artifacts/care-cpu-v47.json':'artifacts/rescue-cpu-v46.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
