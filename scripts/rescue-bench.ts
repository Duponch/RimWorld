...
for(const pairs of [1,15,50]){
  const w=self?selfTendingCamp(pairs*2,250):(feeding?feedingCamp:care?careCamp:rescueCamp)(pairs,250),samples:number[]=[],copies:number[]=[];let carryTicks=0;
...
const report={date:new Date().toISOString(),cpu:cpus()[0]?.model,node:process.version,os:`${platform()} ${release()}`,scope:(self?'Ordinary self-treatment; carryTicks counts work ticks; rescued counts fully self-treated adults. ':feeding?'Physical feeding; carryTicks counts bedside ticks; rescued counts fed patients. ':care?'Physical treatments and medical rest; carryTicks counts active doctor ticks; rescued counts fully treated patients. ':'Physical rescues; carryTicks counts active carrying ticks. ')+'Cleared 250x250 map; concurrent reservations, continuing health/needs; cold and warm ticks included, clones separate',results};
writeFileSync(self?'artifacts/self-tending-cpu-v49.json':feeding?'artifacts/feeding-cpu-v48.json':care?'artifacts/care-cpu-v47.json':'artifacts/rescue-cpu-v46.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
