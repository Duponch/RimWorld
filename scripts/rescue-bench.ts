...
for(const pairs of [1,15,50]){
  const w=(care?careCamp:rescueCamp)(pairs,250),samples:number[]=[],copies:number[]=[];let carryTicks=0;
  if(validateWorld(w).length)throw Error(validateWorld(w).join(';'));
  for(let t=0;t<(care?800:400);t++){
    const now=performance.now();stepWorld(w);samples.push(performance.now()-now);
    carryTicks+=w.pawns.filter(p=>care?p.tend?.phase==='tend':p.rescue?.phase==='carry').length;
  ...
  const rescued=care?w.pawns.reduce((n,p)=>n+p.skills.medicine.xp,0)/175000:w.pawns.filter(p=>p.state==='downed'&&p.need?.kind==='sleep'&&p.need.bedId!==null).length;
...
const report={date:new Date().toISOString(),cpu:cpus()[0]?.model,node:process.version,os:`${platform()} ${release()}`,scope:(care?'Physical treatments and medical rest; carryTicks counts active doctor ticks; rescued counts fully treated patients. ':'Physical rescues; carryTicks counts active carrying ticks. ')+'Cleared 250x250 map; concurrent reservations, continuing health/needs; cold and warm ticks included, clones separate',results};
writeFileSync(care?'artifacts/care-cpu-v47.json':'artifacts/rescue-cpu-v46.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
