...
const workshops=process.argv.includes('--workshops');
const report={workshops,date:new Date().toISOString(),cpu:cpus()[0]!.model,node:process.version,map:250,
...
stepWorld(constructionLoad(3,workshops),100);
for(const count of [3,30,100]) {
  const ticks:number[]=[],snapshots:number[]=[],outcomes=[];
  for(let run=0;run<3;run++) {
    const w=constructionLoad(count,workshops),encoder=new SnapshotEncoder();encoder.encode(w,0,1);
    ...
    if(errors.length||w.jobs.length||materials.wood!==(workshops?Math.ceil(count/2)*75:count*5)||materials.steel!==(workshops?Math.ceil(count/2)*30+Math.floor(count/2)*105:count*25))throw Error(JSON.stringify({errors,jobs:w.jobs.length,materials}));
...
writeFileSync(workshops?'artifacts/stonebench-cpu.json':'artifacts/materials-cpu.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
