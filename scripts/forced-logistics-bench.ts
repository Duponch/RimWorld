...
for(const count of [3,30,100]) {
  ...
  w.pawns=Array.from({length:count},(_,i)=>({...structuredClone(model),id:w.nextId++,x:20+(i%10)*3,z:20+Math.floor(i/10)*3,hunger:100,rest:100,priorities: {research:0, patient:0,bedrest:0,doctor:0,craft:2,mine:0,haul:1,build:0,gather:0,grow:0,cook:cook?1:0}}));
  refreshStock(w);
