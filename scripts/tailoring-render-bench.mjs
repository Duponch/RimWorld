...
try{for(const count of [3,30,100]){
  ...
  await page.addInitScript(data=>localStorage.setItem('lisiere.save.v1',data),serializeWorld(tailoringLoad(count)));
  ...
  const w=JSON.parse(data.world),invalid=validateWorld(w),row={actors:count,adapter,elapsedMs:elapsed,simulatedTicks:w.tick-2000,completed:w.tailoring?.completed,frameMs:stats(data.frames.flatMap(f=>f.interval===null?[]:[f.interval])),frameCpuMs:stats(data.frames.map(f=>f.cpu)),workerBatchStepMs:stats(data.workers),snapshotAdoptionMs:stats(data.snapshots),drawCalls:stats(data.frames.map(f=>f.calls)),pipelines:data.pipelines,stable:data.stable,errors,invalid};report.rows.push(row);
  await page.screenshot({path:`artifacts/tailoring-load-v72-${count}.png`});await page.close();
  if(errors.length||invalid.length||data.pipelines.length||!data.stable||w.tailoring?.completed!==Math.ceil(count/2))throw Error(JSON.stringify(row));
}}
catch(error){report.error=String(error);throw error;}
finally{await browser.close();await writeFile('artifacts/tailoring-render-v72.json',JSON.stringify(report,null,2));}
