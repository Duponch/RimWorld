...
const entries=JSON.parse(manifest).saves;assert.equal(entries.length,11);
...
try{
  ...
  for(const id of ['economie-v105','lievres-v106']){
    ...
    assert.equal(await front.locator('input[name="test-colony"]').count(),11);
    await page.screenshot({path:`artifacts/domestic-public-${id}-catalogue-v106.png`});
    ...
    if(id!=='lievres-v106')await page.locator('#browse-saves').click();
    else{
      await page.keyboard.press('Escape');await page.locator('[data-panel="work"]').click();
      const text=await page.locator('#work-panel').innerText();assert.match(text,/Animaux/);
      ...
      await page.screenshot({path:'artifacts/domestic-public-work-v106.png'});
    }
  }
  assert.equal(await page.evaluate(()=>typeof window.__lisiere),'undefined');
  assert.deepEqual(report.errors,[]);report.passed=true;
}finally{await browser.close();await writeFile('artifacts/domestic-public-v106.json',JSON.stringify(report,null,2)+'\n');}
