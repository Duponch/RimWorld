...
const report={protocol:'V97: ordinary per-camera frustum culling compared to identical ordinary draws without culling. Day/night, hidden/restored/removed foliage and piles; exact pixels required, shadow camera retained.',cases:[],errors:[]};
try{
 ...
 for(const state of ['initial','foliage-hidden','removed','piles','night','restored'])for(const zoom of ['near','middle','far']){
  const captures=await page.evaluate(async({state,zoom})=>{
   ...
   const take=async(enabled,conservative=enabled)=>{
    v.landscape.refresh(conservative);v.landscape.isBundleGroup=enabled;
   ...
   const originalCulling=await take(false),plain=await take(false,true),retained=originalCulling;
  ...
  report.cases.push({state,zoom,unculled:a,culled:b,exact:a===b,originalCulling:hash(captures.originalCulling),originalCullingExact:hash(captures.originalCulling)===a,changedPixels:captures.changedPixels,maxChannelDifference:captures.maxChannelDifference,rasterEquivalent:captures.changedPixels===0});
  if(a!==b){await writeFile(`artifacts/landscape-v97-${state}-${zoom}-plain.png`,Buffer.from(captures.plain.split(',')[1],'base64'));await writeFile(`artifacts/landscape-v97-${state}-${zoom}-retained.png`,Buffer.from(captures.retained.split(',')[1],'base64'));}
 }
}finally{await browser.close();await writeFile('artifacts/landscape-equivalence-v97.json',JSON.stringify(report,null,2));}
console.log(JSON.stringify({cases:report.cases.length,failed:report.cases.filter(c=>!c.rasterEquivalent),errors:report.errors}));
