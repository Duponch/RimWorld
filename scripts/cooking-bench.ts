...
for(const pawns of populations) {
  ...
  for(let repeat=0;repeat<repeats;repeat++) {
    ...
    result={hash:hashWorld(w),cooked,expiryTickMs,spoiled:{...w.spoiled},crops:w.resources.filter(r=>r.kind==='rice').length,walls:w.structures.filter(s=>s.kind==='wall').length,activePawnTicks:active,remainingRaw:w.piles.filter(p=>p.item==='rice'||p.item==='berries').reduce((n,p)=>n+p.quantity,0),remainingMeals:w.piles.filter(p=>p.item==='simple-meal').reduce((n,p)=>n+p.quantity,0),searchTotals,worst};
    if(result.remainingRaw+w.spoiled.rice+w.spoiled.berries+cooked*10!==Math.ceil(pawns/5)*110)throw new Error('Recipe input conservation failed');
  }
...
const report={prioritized,date:new Date().toISOString(),cpu:cpus()[0]?.model,node:process.version,map:'250x250 seed 42',expireAt,conditions:`${repeats} runs × ${ticks} ticks; setup, validation and diagnostic aggregation excluded; search-counter collection inside stepWorld is timed; needs active, all workers cook/build/haul/grow; no warmup (includes first assignments); expiry injection is a separate stress fixture, not an equivalent gameplay baseline`,rows};
