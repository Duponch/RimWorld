...
try{
  ...
  await page.screenshot({path:'artifacts/netlify-v92-home.png'});
  ...
  await page.screenshot({path:'artifacts/netlify-v92-colony.png'});
  ...
  const saved=await page.evaluate(()=>{const w=JSON.parse(localStorage.getItem('lisiere.save.v1'));return {schema:w.schemaVersion,tick:w.tick,pawns:w.pawns.length};});
  ...
  result.assets=await page.evaluate(async()=>{
    const rows=[];for(const name of ['planet','panel-frame','icons','portraits']){
  ...
  result.errors=errors;await writeFile('artifacts/netlify-smoke-v92.json',JSON.stringify(result,null,2)+'\n');
  await copyFile('artifacts/netlify-latest.json','artifacts/netlify-v92.json');await browser.close();
