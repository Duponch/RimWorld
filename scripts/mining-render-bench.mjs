...
const before=method==='setWorld'?${stonecutting?"this.world?.piles.reduce((n,p)=>n+(p.kind==='blocks'?p.quantity/20:0),0)??0":"this.world?.jobs.filter(j=>j.kind==='"+(workshops?'stonecutter':'mine')+"').length??0"}:0,start=performance.now();
...
try {
 for(const count of (process.env.MINING_COUNTS??'3,30,100').split(',').map(Number)) {
  ...
  await page.addInitScript(saved=>localStorage.setItem('lisiere.save.v1',saved),serializeWorld(stonecutting?stonecuttingLoad(count):workshops?constructionLoad(count,true):miningLoad(count,steel)));
  ...
  await page.waitForFunction(n=>window.__miningBench.events.reduce((sum,e)=>sum+e.mined,0)>=n,count*(stonecutting?3:workshops?1:4),{timeout:90000,polling:250});
  await page.locator('[data-speed="0"]').click();
  const b=await page.evaluate(()=>{const b=window.__miningBench,v=b.view;b.active=false;return {frames:b.frames,adoptions:b.adoptions,snapshots:b.snapshots,longFrames:b.longFrames,events:b.events,stableRock:b.initial.rock===v.rocks.mesh.geometry&&b.initial.position===v.rocks.mesh.geometry.getAttribute('position')&&b.initial.index===v.rocks.mesh.geometry.index,stableGround:JSON.stringify(b.initial.ground)===JSON.stringify(v.terrainGroup.children.map(g=>g.uuid)),blocks:v.world.piles.reduce((n,p)=>n+(p.kind==='blocks'?p.quantity:0),0),steelUnits:v.world.piles.reduce((n,p)=>n+(p.item==='steel'?p.quantity:0),0),chunks:v.world.piles.filter(p=>p.kind==='chunk').length,workshops:v.world.structures.filter(s=>s.kind==='stonecutter').length};});
  const phase={blocks:b.blocks,workshops:b.workshops,count,adapter,errors,frames:stats(b.frames.flatMap(f=>f.interval===null?[]:[f.interval])),frameCpu:stats(b.frames.map(f=>f.cpu)),adoptions:stats(b.adoptions),drawCalls:stats(b.frames.map(f=>f.calls)),excavationFrames:stats(b.frames.filter(f=>b.events.some(e=>f.at>=e.at&&f.at<e.at+150)).flatMap(f=>f.interval===null?[]:[f.interval])),mined:b.events.reduce((n,e)=>n+e.mined,0),steelUnits:b.steelUnits,chunks:b.chunks,stableRock:b.stableRock,stableGround:b.stableGround};
  ...
  if(stonecutting&&b.blocks!==count*60||workshops&&b.workshops!==count||steel&&b.steelUnits!==count*160||errors.length||!b.stableRock||!b.stableGround||phase.shadowPreparation&&phase.pipelines.length)throw new Error(JSON.stringify(phase));
 }
