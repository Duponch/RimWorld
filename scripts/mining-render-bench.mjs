...
try {
 for(const count of (process.env.MINING_COUNTS??'3,30,100').split(',').map(Number)) {
  ...
  if(stonecutting&&b.blocks!==count*60||workshops&&b.workshops!==count||machinery&&b.componentUnits!==count*8||steel&&b.steelUnits!==count*160||errors.length||!b.stableRock||!b.stableGround||phase.shadowPreparation&&phase.pipelines.length)throw new Error(JSON.stringify(phase));
 }
