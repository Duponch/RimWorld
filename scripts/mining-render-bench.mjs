...
const probe=`
window.__miningBench={active:false,view:null,previous:null,frames:[],adoptions:[],events:[],snapshots:[],longFrames:[],pipelines:[],preparations:[]};
...
try {
 for(const count of (process.env.MINING_COUNTS??'3,30,100').split(',').map(Number)) {
  ...
  await page.addInitScript(installGpuCallProbe);
  ...
  if(errors.length||!b.stableRock||!b.stableGround||phase.shadowPreparation&&phase.pipelines.length)throw new Error(JSON.stringify(phase));
 }
