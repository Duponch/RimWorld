...
`;
const trade=process.env.TRADE==='1',environment=trade||process.env.ENVIRONMENT==='1',prisoners=!environment&&process.env.PRISONERS==='1',energy=environment||prisoners||process.env.ENERGY==='1',foodChain=energy||process.env.FOOD_CHAIN==='1',infection=!foodChain&&process.env.INFECTIONS==='1',hunting=process.env.HUNTING==='1';
...
try{for(const count of trade?[100]:[3,30,100]){
  ...
  const fixture=makeLoad(count),tradeInitial=trade?tradeLoadInitial(fixture):undefined,environmentInitial=environment?environmentLoadInitial(fixture):undefined,prisonInitial=prisoners?prisonLoadInitial(fixture):undefined,initialCropIds=foodChain?foodChainCropIds(fixture):[];
