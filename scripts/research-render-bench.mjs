...
async function frameFoodChain(page,detail=false) {
  return page.evaluate(detail=>{
    ...
    const food=w.structures.filter(s=>['fueled-stove','electric-stove','butcher-table','wood-generator'].includes(s.kind));
...
try{for(const count of [3,30,100]){
  ...
  const foodCamera=foodChain?await frameFoodChain(page):undefined;
  ...
  if(foodChain){row.closeCamera=await frameFoodChain(page,true);await page.evaluate(()=>new Promise(resolve=>{let n=30;function frame(){if(!--n)resolve();else requestAnimationFrame(frame);}requestAnimationFrame(frame);}));await page.screenshot({path:`artifacts/${label}-stations-${version}-${count}.png`});}
  await page.close();
