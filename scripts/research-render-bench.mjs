...
async function frameFoodChain(page,detail=false,energy=false) {
  return page.evaluate(({detail,energy})=>{
    ...
    const food=w.structures.filter(s=>['fueled-stove','electric-stove','butcher-table','wood-generator',...energy?['solar-generator','battery','power-switch','power-conduit','cooler']:[]].includes(s.kind));
...
try{for(const count of [3,30,100]){
  ...
  const fixture=makeLoad(count),prisonInitial=prisoners?prisonLoadInitial(fixture):undefined,initialCropIds=foodChain?foodChainCropIds(fixture):[];
  ...
  const foodCamera=foodChain?await frameFoodChain(page,false,energy):undefined;
  ...
  const w=JSON.parse(data.world),invalid=validateWorld(w),foodErrors=prisonInitial?prisonLoadOutcomeErrors(w,initialCropIds,prisonInitial):energy?energyLoadOutcomeErrors(w,initialCropIds):foodChain?foodChainOutcomeErrors(w,initialCropIds):[],row={actors:count,...prisonInitial?{totalActors:w.pawns.length,prison:prisonLoadSummary(w,initialCropIds,prisonInitial)}:{},...foodChain?{foodChain:prisonInitial?prisonLoadSummary(w,initialCropIds,prisonInitial).energy.food:foodChainSummary(w,initialCropIds),...energy?{energy:prisonInitial?prisonLoadSummary(w,initialCropIds,prisonInitial).energy:energyLoadSummary(w,initialCropIds)}:{},foodErrors}:{},infected:w.pawns.filter(p=>p.health?.infections?.cases.length).length,treatedInfections:w.pawns.reduce((n,p)=>n+(p.health?.infections?.cases.filter(c=>c.tend).length??0),0),animals:w.wildlife?.animals.length??0,animalNutrition:w.wildlife?.eatenNutrition??0,hunted:w.hunting?.completed??0,corpses:w.piles.filter(p=>p.kind==='corpse').length,animalDead:w.wildlife?.animals.filter(a=>a.state==='dead').length??0,colonistInjured:w.pawns.filter(p=>p.health?.injuries.length).length,adapter,elapsedMs:elapsed,simulatedTicks:w.tick-startTick,completed:w.tailoring?.completed??0,research:w.research?.points,exposed:w.pawns.filter(p=>p.health?.heatstroke||p.health?.hypothermia).length,coolers:w.structures.filter(s=>s.kind==='cooler').length,frameMs:stats(data.frames.flatMap(f=>f.interval===null?[]:[f.interval])),frameCpuMs:stats(data.frames.map(f=>f.cpu)),workerBatchStepMs:stats(data.workers),snapshotAdoptionMs:stats(data.snapshots),drawCalls:stats(data.frames.map(f=>f.calls)),pipelines:data.pipelines,stable:data.stable,errors,invalid};report.rows.push(row);
  ...
  if(foodChain){row.closeCamera=await frameFoodChain(page,true,energy);await page.evaluate(()=>new Promise(resolve=>{let n=30;function frame(){if(!--n)resolve();else requestAnimationFrame(frame);}requestAnimationFrame(frame);}));await page.screenshot({path:`artifacts/${label}-stations-${version}-${count}.png`});}
  await page.close();
