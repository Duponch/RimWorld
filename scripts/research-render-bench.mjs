import { wildlifeLoad,injuredWildlifeLoad,meleeWildlifeLoad } from '../tests/scenarios/wildlife-load.ts';
...
const wild=process.env.WILDLIFE==='1',cold=!wild&&process.env.COLD_STORE==='1',hot=!wild&&!cold&&process.env.HEATWAVE==='1',makeLoad=animalMelee?meleeWildlifeLoad:animalCombat?injuredWildlifeLoad:wild?wildlifeLoad:cold?coldStoreLoad:hot?heatwaveLoad:researchLoad,version=process.env.VALIDATION_VERSION??(wild?'v76':cold?'v75':hot?'v74':'v73'),label=animalMelee?'animal-melee':animalCombat?'animal-combat':wild?'wildlife':cold?'cold-store':hot?'heatwave':'research',startTick=hot?4000:2000;
const report={date:new Date().toISOString(),cpu:os.cpus()[0].model,viewport:{width:1440,height:1000},protocol:(animalMelee?'One miner in six is drafted against an adjacent hare; the other workers retain their physical research/crafting/mining tasks. Melee injuries and outcomes are real. ':'')+(animalCombat?'Half of the hares have real tail gunshot injuries, blood loss and an initial bounded escape; no corpse transport. ':'')+(wild?'Same number of wild hares as colonists, half start hungry, one third tired; vegetation and piles consumed physically. ':'')+(cold?'1/6/20 powered cold rooms and rice stores; one fifth of actors start at 34% hypothermia and physically seek warmth; ':'')+(hot?'Heatwave plateau 17°C offset, tick 4000–4650; half shirt-wearing actors start at 34% heatstroke, other half tribalwear; ':'')+'Native Chromium WebGPU; natural 250², physical apparel on 3/30/100 actors. One third research at physical benches; others gather/craft tribalwear or mine/chop. Real worker at requested 6× for 650 ticks, 90 warmup frames, one run each; no full World export during timing. Worker samples are published batch step averages, not an independent per-tick percentile.',wildlife:wild,coldStore:cold,heatwave:hot,rows:[]};
