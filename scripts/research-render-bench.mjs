...
import { hygieneLoad,hygieneLoadInitial,hygieneLoadSummary,hygieneLoadOutcomeErrors,HYGIENE_PROTOCOL } from '../tests/scenarios/hygiene-load.ts';
import { habitatApparelLoad,habitatApparelLoadInitial,habitatApparelLoadSummary,habitatApparelLoadOutcomeErrors,HABITAT_APPAREL_PROTOCOL } from '../tests/scenarios/habitat-apparel-load.ts';
import { tradeLoad,tradeLoadInitial,tradeLoadSummary,tradeLoadOutcomeErrors,tradeEnvironmentView,TRADE_PROTOCOL } from '../tests/scenarios/trade-load.ts';
...
async function frameFoodChain(page,detail=false,energy=false,environment=false,habitatApparel=false) {
  return page.evaluate(({detail,energy,environment,habitatApparel})=>{
    ...
    const food=w.structures.filter(s=>['fueled-stove','electric-stove','butcher-table','wood-generator',...energy?['solar-generator','battery','power-switch','power-conduit','cooler']:[],...environment?['wind-turbine','heater']:[],...habitatApparel?['table-long','dining-chair','table-square','stool','armchair','end-table','dresser','flower-pot','tailor-bench','electric-tailor-bench']:[]].includes(s.kind));
    ...
    return {view:detail?'food-close':'whole-workload',projection:rig.mode,bounds,zoom:camera.zoom,verticalSpan:rig.span,target:controls.target.toArray(),position:camera.position.toArray(),safeCanvasRect:safe,initialAnchors:detail?null:{pawns:w.pawns.length,researchBenches:research.length,foodStructures:food.length,growingCells:cells.length,habitatFurniture:habitatApparel?food.filter(s=>['table-long','dining-chair','table-square','stool','armchair','end-table','dresser'].includes(s.kind)).length:0,habitatFlowers:habitatApparel?food.filter(s=>s.kind==='flower-pot').length:0,habitatTailorBenches:habitatApparel?food.filter(s=>s.kind==='tailor-bench'||s.kind==='electric-tailor-bench').length:0},foliage:'unchanged'};
  },{detail,energy,environment,habitatApparel});
}
const prefix=`
...
`;
const habitatApparel=process.env.HABITAT_APPAREL==='1',hygiene=habitatApparel||process.env.HYGIENE==='1',trade=hygiene||process.env.TRADE==='1',environment=trade||process.env.ENVIRONMENT==='1',prisoners=!environment&&process.env.PRISONERS==='1',energy=environment||prisoners||process.env.ENERGY==='1',foodChain=energy||process.env.FOOD_CHAIN==='1',infection=!foodChain&&process.env.INFECTIONS==='1',hunting=process.env.HUNTING==='1';
...
try{for(const count of habitatApparel||trade?[100]:[3,30,100]){
  ...
  const fixture=makeLoad(count),habitatInitial=habitatApparel?habitatApparelLoadInitial(fixture):undefined,hygieneInitial=hygiene&&!habitatApparel?hygieneLoadInitial(fixture):undefined,tradeInitial=trade&&!habitatApparel?tradeLoadInitial(fixture):undefined,environmentInitial=environment&&!habitatApparel?environmentLoadInitial(fixture):undefined,prisonInitial=prisoners?prisonLoadInitial(fixture):undefined,initialCropIds=foodChain?foodChainCropIds(fixture):[];
  if(process.env.ECONOMY==='1')adoptColonyEconomy(fixture);
  ...
  const foodCamera=foodChain?await frameFoodChain(page,false,energy,environment,habitatApparel):undefined;
  ...
  if(foodChain){row.closeCamera=await frameFoodChain(page,true,energy,environment,habitatApparel);await page.evaluate(()=>new Promise(resolve=>{let n=30;function frame(){if(!--n)resolve();else requestAnimationFrame(frame);}requestAnimationFrame(frame);}));await page.screenshot({path:`artifacts/${label}-stations-${version}-${count}.png`});}
  await page.close();
