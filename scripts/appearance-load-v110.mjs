...
try {
 for(const version of ['V109','V110','V110','V109']){
  ...
  if(version==='V109')await page.route('**/src/render/ColonyRenderer.ts*',async route=>{const response=await route.fetch(),body=await response.text();assert.match(body,/\/src\/render\/PawnLayer.ts/);await route.fulfill({response,body:body.replace(/\/src\/render\/PawnLayer.ts[^"']*/g,'/tmp/appearance-baseline/PawnLayer.ts')});});
 ...
 writeFileSync('artifacts/appearance-load-v110.json',JSON.stringify({date:new Date().toISOString(),protocol:'Native WebGPU, 1440x1000, mixed-100 immutable save, all-map view, V109/V110/V110/V109 appearance renderers sequentially; same current simulation and immutable scene, pause and 6x windows of six seconds after one second settling. RAF wall time and render CPU are separate; not a GPU timer or simulation microbenchmark.',reports,errors},null,2)+'\n');
