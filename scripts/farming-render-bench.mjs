...
const report={date:new Date().toISOString(),cpu:os.cpus()[0].model,protocol:'250² seed 42, 1440×1000, hardware WebGPU, paused. Same cleared field and camera: 0/1000/0 rice plants. 30 transition + 90 warm frames, then >=300 frames and 6 s. CPU submission is not GPU time. Synthetic presentation fixture, no gameplay throughput claim.',phases:[],errors:[]};
...
try {
  ...
  for(const count of [0,1000,0]) {
    const result=await page.evaluate(count=>new Promise(done=>{
      const b=window.__farmBench,v=b.view,w=structuredClone(b.base);
      for(let i=0;i<count;i++)w.resources.push({id:w.nextId++,kind:'rice',x:110+i%32,z:110+Math.floor(i/32),amount:6,growth:1,growthTick:w.tick});
      const start=performance.now();v.setWorld(w);b.setWorldMs=performance.now()-start;
    ...
    const phase={count,setWorldMs:result.setWorldMs,frameMs:frameMs(result.frames),cpuMs:stats(result.frames.map(r=>r.cpu)),calls:stats(result.frames.map(r=>r.calls)),triangles:stats(result.frames.map(r=>r.triangles)),transitionFrameMs:frameMs(result.transition),transitionCpuMs:stats(result.transition.map(r=>r.cpu))};
    report.phases.push(phase);console.log(`${count} rice: ${phase.calls.mean} calls, p95 ${phase.frameMs.p95.toFixed(2)} ms, setWorld ${phase.setWorldMs.toFixed(2)} ms`);
    if(count)await page.screenshot({path:'artifacts/farming-benchmark.png'});
  }
  if(report.errors.length)throw Error(report.errors.join('\n'));
} finally {await browser.close();await writeFile('artifacts/farming-render-benchmark.json',JSON.stringify(report,null,2)+'\n');}
