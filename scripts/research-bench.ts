...
const hunting=process.env.HUNTING==='1';
const animalMelee=process.env.ANIMAL_MELEE==='1',animalCombat=process.env.ANIMAL_COMBAT==='1'||animalMelee;
const wild=process.env.WILDLIFE==='1',cold=!wild&&process.env.COLD_STORE==='1',hot=!wild&&!cold&&process.env.HEATWAVE==='1',makeLoad=hunting?huntingWildlifeLoad:animalMelee?meleeWildlifeLoad:animalCombat?injuredWildlifeLoad:wild?wildlifeLoad:cold?coldStoreLoad:hot?heatwaveLoad:researchLoad,version=process.env.VALIDATION_VERSION??(wild?'v76':cold?'v75':hot?'v74':'v73'),label=hunting?'hunting':animalMelee?'animal-melee':animalCombat?'animal-combat':wild?'wildlife':cold?'cold-store':hot?'heatwave':'research',startTick=hot?4000:2000;
