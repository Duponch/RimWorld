...
for(const count of [3,30,100]){
  ...
  if(!hot&&(w.tailoring?.completed??0)!==Array.from({length:count},(_,i)=>i).filter(i=>i%2===0&&i%3!==0).length)throw Error(`Incomplete garments ${count}: ${w.tailoring?.completed}`);
  rows.push({actors:count,tickMs:stats(ticks),steadyTickMs:stats(ticks.slice(20)),snapshotMs:stats(snapshots),completed:w.tailoring?.completed??0,research:w.research!.points,exposed:w.pawns.filter(p=>p.health?.heatstroke).length,mined:w.tiles.filter(t=>t.terrain==='rough-stone').length});
}
const report={date:new Date().toISOString(),cpu:cpus()[0]!.model,os:platform()+' '+release(),node:process.version,protocol:(hot?'Heatwave plateau 17°C offset, tick 4000–4650; half shirt-wearing actors start at 34% heatstroke, other half tribalwear; ':'')+'Natural 250² map, 3/30/100 actors with physical apparel. One third research at physical benches; others gather/craft tribalwear or mine/chop. 650 ticks, 100 separate warmup ticks, encoding every five ticks. Single pass, CPU only; no 6× guarantee.',heatwave:hot,rows};
