import { initializeCampTraits } from '../src/sim/traits.ts';
import { enableRaids } from '../src/sim/raids.ts';
import { barrierLoad } from '../tests/scenarios/barrier-load.ts';
...
for(const count of [3,30,100]) {
  const {world:w,pairs}=(barriers?barrierLoad(count):pursuit?pursuitLoad(count):automatic?automaticLoad(count):melee?meleeLoad(count):hostileTargets?encounterLoad(count):shootingLoad(count,movingTargets)),commands:number[]=[],initialResources=w.resources.length,initialRocks=w.tiles.filter(t=>t.terrain==='rock').length;
  if(process.env.RAIDS==='1'){enableRaids(w);w.raids!.nextCheck=w.tick+1;}
  if(process.env.APPAREL==='1')dressLoad(w);
  if(process.env.TRAITS==='1')initializeCampTraits(w);
