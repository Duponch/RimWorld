...
for(const count of [3,30,100]) {
  ...
  results.push({pursuit,automatic,actors:count,pairs:pairs.length,movingTargets,hostileTargets,slowedPawns:slowed.size,retimedEdges:retimed.size,accepted,refused,emissions:emissions.size,impacts,commandsMs:stats(commands),allTickMs:stats(allTickMs),initialTickMs:stats(allTickMs.slice(0,20)),mixedTickMs:stats(mixedMs),quietTickMs:stats(quietMs),activeCombatTickMs:stats(activeMs),encodeMs:stats(encodedMs),dead:w.pawns.filter(p=>p.state==='dead').length,downed:w.pawns.filter(p=>p.state==='downed').length,mixedExcavatedTiles:initialRocks-w.tiles.filter(t=>t.terrain==='rock').length,mixedResourcesRemoved:initialResources-w.resources.length,quietResourcesRemoved:initialResources-quiet.resources.length});
}
