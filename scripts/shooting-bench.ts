...
for(const count of [3,30,100]) {
  const {world:w,pairs}=(melee?meleeLoad(count):hostileTargets?encounterLoad(count):shootingLoad(count,movingTargets)),commands:number[]=[],initialResources=w.resources.length,initialRocks=w.tiles.filter(t=>t.terrain==='rock').length;
  ...
  for(let tick=0;tick<240;tick++) {
    if(tick%60===0)for(const [pawnId,targetId] of pairs){const t=performance.now();const result=applyCommand(w,{type:melee?'melee':'shoot',pawnIds:[pawnId],targetId});commands.push(performance.now()-t);result.ok?accepted++:refused++;}
