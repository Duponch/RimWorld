...
for(const count of [3,30,100]) {
  const {world:w,pairs}=(automatic?automaticLoad(count):melee?meleeLoad(count):hostileTargets?encounterLoad(count):shootingLoad(count,movingTargets)),commands:number[]=[],initialResources=w.resources.length,initialRocks=w.tiles.filter(t=>t.terrain==='rock').length;
