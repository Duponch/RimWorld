...
for(const count of [3,30,100]) {
  const {world:w,pairs}=shootingLoad(count),commands:number[]=[],initialResources=w.resources.length;
  const quiet=structuredClone(w),mixedMs:number[]=[],quietMs:number[]=[],encodedMs:number[]=[],activeMs:number[]=[];
  const encoder=new SnapshotEncoder();encoder.encode(w,0,6);let accepted=0,refused=0,impacts=0,emissions=new Set<number>();
