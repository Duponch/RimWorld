...
const fixture = JSON.parse(await readFile('artifacts/furniture-transfer-load.json','utf8'));
const probe = `
...
try {
 ...
 await page.waitForFunction(()=>!window.__lisiere.world.jobs.some(j=>j.kind==='install')&&!window.__lisiere.world.packed.length,undefined,{timeout:60000,polling:250});
 await page.evaluate(()=>new Promise(resolve=>{let n=0;function f(){if(++n===60)resolve();else requestAnimationFrame(f);}requestAnimationFrame(f);}));
 ...
 report.outcome=await page.evaluate(()=>{const w=window.__lisiere.world;return {tick:w.tick,wood:w.stock.wood,pending:w.jobs.filter(j=>j.kind==='install').length,packed:w.packed.length,installed:w.structures.length};});
 if(report.errors.length||report.outcome.pending||report.outcome.packed||report.outcome.installed!==100)throw new Error(JSON.stringify(report));
} finally {await browser.close();}
await writeFile('artifacts/furniture-transfer-render-'+variant+'.json',JSON.stringify(report,null,2)+'\n');
