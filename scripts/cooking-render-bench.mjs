...
try {
  for(const count of populations) {
    ...
    const initial=cookingFixture(count);
    ...
    for(const view of ['local','overview']) {
      ...
      await page.screenshot({path:`artifacts/cooking-${count}-${view}.png`});
    }
    await page.close();
  }
