...
const recreation=process.argv[4]==='recreation',occupancy=process.argv[4]==='occupancy';
const report={workload:recreation?'recreation':occupancy?'occupancy':'cooking',date:new Date().toISOString(),cpu:cpus()[0].model,protocol:'Native Chromium WebGPU, 1440×1000, natural 250² with synthetic camps; real worker at UI speed 6; 60 warmup frames then >=300 frames and >=5 seconds per phase. Local/overview phases advance the simulation successively, so they are workload observations, not identical-state camera A/B. RAF includes scheduling; frame CPU is submission, not GPU execution. No parallel tests or CPU benchmarks.',phases:[],errors:[]};
...
try {
  for(const count of populations) {
    ...
    // Synthetic surface stress: one table and one stocked stool per camp.
    // This changes routes and is not an identical-state historical A/B.
    if(occupancy)for(const pile of initial.piles.filter(p=>p.owner.type==='ground'&&(p.item==='rice'||p.item==='berries'))) {
      const kind=pile.item==='rice'?'stool':'table',cell=pile.owner;
      initial.structures.push({id:initial.nextId++,kind,x:cell.x,z:cell.z,orientation:0,footprint:'standard'});
    ...
    for(const view of ['local','overview']) {
      ...
      if(occupancy)entry.surfaceFurniture=state.structures.filter(s=>s.kind==='stool'||s.kind==='table').length;
      ...
      await page.screenshot({path:`artifacts/${recreation?'recreation':occupancy?'occupancy':'cooking'}-${count}-${view}.png`});
    }
    await page.close();
  }
