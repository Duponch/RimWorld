...
// Freeze the published V108 human mesh, retaining the current simulation in both arms.
mkdirSync('tmp/appearance-baseline',{recursive:true});
for(const name of ['PawnLayer.ts','pawn-geometry.ts']){let source=execFileSync('git',['show',`f4a2dca:src/render/${name}`],{encoding:'utf8'}).replaceAll("'../","'/src/").replaceAll("'./","'/src/render/");if(name==='PawnLayer.ts')source=source.replace("'/src/render/pawn-geometry'","'/tmp/appearance-baseline/pawn-geometry.ts'");writeFileSync(`tmp/appearance-baseline/${name}`,source);}
...
try {
 for(const version of ['V108','V109','V109','V108']){
  ...
  if(version==='V108')await page.route('**/src/render/ColonyRenderer.ts*',async route=>{const response=await route.fetch(),body=await response.text();assert.match(body,/\/src\/render\/PawnLayer.ts/);await route.fulfill({response,body:body.replace(/\/src\/render\/PawnLayer.ts[^"']*/g,'/tmp/appearance-baseline/PawnLayer.ts')});});
 ...
 writeFileSync('artifacts/appearance-load-v109.json',JSON.stringify({date:new Date().toISOString(),protocol:'Native WebGPU, 1440x1000, mixed-100 immutable save, all-map view, V108/V109/V109/V108 appearance renderers sequentially; same current simulation and immutable scene, pause and 6x windows of six seconds after one second settling. RAF wall time and render CPU are separate; not a GPU timer or simulation microbenchmark.',reports,errors},null,2)+'\n');
