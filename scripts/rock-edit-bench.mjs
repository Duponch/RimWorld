...
const report={timestamp:new Date().toISOString(),cpu:os.cpus()[0].model,viewport:{width:1440,height:1000},errors:[],protocol:'Hardware WebGPU, 250x250 seed 42, paused, focused outcrop. Twelve diagnostic rock-to-soil edits at 300 ms spacing after 90 warmup frames, one browser promise. These are presentation tests, not playable mining.'};
try {
  ...
  await page.screenshot({path:'artifacts/rocks-close.png'});
  report.edits=await page.evaluate(()=>new Promise(resolve=>{
    ...
    function frame(now){
      ...
      if(now-last>=300&&records.length<candidates.length){
        ...
        const started=performance.now();v.setWorld(world,false,0);records.push({cell,cpuMs:performance.now()-started,updatedCells:v.rocks.stats.updatedCells,indexCount:v.rocks.stats.indexCount});
      }
  ...
  await page.screenshot({path:'artifacts/rocks-excavated.png'});
...
await writeFile('artifacts/rock-edit-benchmark.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
