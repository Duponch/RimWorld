...
if(reference&&!['v96','v98','v113','v114','v115'].includes(reference))throw Error('Unknown reference; archive the documented source revision first');
...
const grassPreference=process.env.PERF_GRASS;
if(grassPreference&&!['on','off'].includes(grassPreference))throw Error('PERF_GRASS must be on or off');
...
try{
 ...
 if(grassPreference)await page.addInitScript(enabled=>localStorage.setItem('lisiere.presentation.ground-grass.v1',enabled?'true':'false'),grassPreference==='on');
 ...
 await page.route('**/src/main.ts*',async route=>{const response=await route.fetch(reference?{url:`${origin}/tmp/perf-${reference}/src/main.ts`}:baseline?{url:`${origin}/tmp/perf-v94/src/main.ts`}:{});await route.fulfill({response,body:prefix+await response.text()+suffix});});
 await page.goto(`${origin}/?scenario=crashlanded&e2e&size=250&seed=42`);
 ...
 report.baseline=reference?(reference==='v115'?'52822fa':reference==='v114'?'0e7c0c4':reference==='v113'?'9866b60':reference==='v98'?'d5cb6f7':'c9010ec'):baseline?'f993e3a':bundleBaseline?'5bd1af3 (render adapter bypassed)':null;
 ...
 report.groundGrass=grassPreference??'browser default';
