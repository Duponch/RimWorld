...
export function updateWildlifePanel(root:HTMLElement,world:World,focus:(id:number)=>void,enable:()=>void,selected:readonly number[]=[],shoot?:(id:number)=>void):void {
  if(!root.querySelector('[data-fauna-list]')) {
    root.innerHTML='<p>Lièvres sauvages · herbivores. Les portes fermées les arrêtent. Les blessures affectent leurs capacités ; les impacts peuvent les faire fuir.</p><p class="muted">Pour tirer : sélectionnez et mobilisez un colon armé, puis utilisez Tirer dans cette liste. La chasse automatique, le transport des dépouilles et la boucherie restent à venir.</p><button data-fauna-enable>Introduire la faune dans cette ancienne partie</button><div data-fauna-list></div>';
  ...
  if(list.dataset.ids!==signature){
    list.dataset.ids=signature;list.replaceChildren(...animals.map(a=>{
      ...
      row.append(button,document.createElement('span'),attack,health);return row;
    }));
  }
  for(const a of animals){
    const state=a.state==='moving'&&!a.path.length&&!a.meal&&(!a.motion||a.motion.end<=world.tick)?'idle':a.state;
    list.querySelector(`[data-animal="${a.id}"] span`)!.textContent=` · ${a.sex==='female'?'Femelle':'Mâle'} · ${a.flee?'Fuit':labels[state]}${a.meal&&state==='moving'?' vers sa nourriture':''} · ${a.x}, ${a.z} `;
