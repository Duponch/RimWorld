...
import { plantGrowth, harvestable, berryYield, plantResting, plantTemperatureFactor, sowingTemperatureAllowed } from '../sim/plants';
...
export function plantInspection(world:World,plant:Resource):string {
  ...
  return ` · Croissance ${Math.floor(plantGrowth(world,plant)*100)} % · ${harvestable(world,plant)?`Récolte : environ ${Math.round(berryYield(world,plant))} ${plant.kind==='rice'?'riz':'baies'}`:'Pas encore récoltable'} · ${constraints.length?constraints.join(' · '):'Croissance diurne'}`;
}
