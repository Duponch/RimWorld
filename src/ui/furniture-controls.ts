...
export function furnitureControls(panel:HTMLElement,world:()=>World|undefined,cell:()=>Cell|undefined,send:(command:Command)=>void,place:(id:number)=>void):void {
  for(const [id,label] of [['cell-uninstall','Désinstaller'],['cell-install','Réinstaller']]) {
    ...
    button.onclick=()=>{
      ...
      const installed=w.structures.find(s=>footprintCells(s).some(p=>p.x===c.x&&p.z===c.z));
...
export function updateFurnitureControls(panel:HTMLElement,world:World,cell:Cell):void {
  const object=world.structures.find(s=>footprintCells(s).some(p=>p.x===cell.x&&p.z===cell.z)),pack=packedAt(world,cell),target=pack?.building??object;
  const busy=(id:number)=>world.jobs.some(j=>j.furniture?.structureId===id||j.deconstruction?.structureId===id);
