...
import { BeautyMapCache, BEAUTY_BAND_LABEL } from '../sim/room-beauty';
...
export class RoomInspection {
  ...
  private readonly beauty = new BeautyMapCache();
  ...
  update(panel: HTMLElement, world: World, cell: Cell): void {
    ...
    // Worker snapshots may preserve array identities for unchanged fields; an
    // inspection refresh is infrequent and must prefer current physical state.
    this.beauty.invalidateAll();
    const beauty=captureWorldBeauty(world,environment.topology,this.beauty).room(cell);
