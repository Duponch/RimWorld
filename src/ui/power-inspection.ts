...
import { isElectrical, isPowerActive, powerWatts, LAMP_DEMAND } from '../sim/power-rules';
...
export function powerInspection(world:World,s:Structure):string {
  ...
  const required=group?.filter(a=>a.kind==='standing-lamp').length??0;
  ...
  return ` · ${state}. ${s.kind==='wood-generator'?`Production ${Math.max(0,powerWatts(s))} W · 22 bois/jour`:`Demande ${LAMP_DEMAND} W${parent?` · raccordée au générateur ${parent.x}, ${parent.z}`:''}`} · Réseau : ${supply} W produits, ${used} W utilisés (${required*LAMP_DEMAND} W demandés). Aucun stockage électrique.`;
}
