...
export function updateUnfinishedInspection(panel:HTMLElement,world:World,pile:MaterialPile|undefined,send:(c:Command)=>void):void {
  ...
  root.querySelector('p')!.textContent=`Tenue inachevée · ${Math.floor(100*u.progress/productionWorkTotal(u.recipe))} % · auteur : ${author?.name??'inconnu'} · 60 tissus engagés. L’annulation récupère environ 75 % du tissu.`;
