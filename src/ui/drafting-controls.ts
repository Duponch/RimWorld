...
export const draftLabel=(p:Pawn):string=>p.need?.kind==='sleep'?'Mobilisé · effondré de fatigue':p.draft?.target&& (p.x!==p.draft.target.x||p.z!==p.draft.target.z||p.moveCooldown>0)?`Mobilisé · vers ${p.draft.target.x}, ${p.draft.target.z}`:'Mobilisé · attend les ordres';
...
export function createDraftControls(parent:HTMLElement,current:()=>Pawn[],send:(c:Command)=>void):void {
  ...
  box.innerHTML='<button id="toggle-draft" class="secondary-action">Mobiliser · R</button><button id="stop-draft" class="secondary-action" hidden>Arrêter le déplacement</button><p id="draft-help" class="muted"></p>';
...
export function updateDraftControls(parent:HTMLElement,pawns:Pawn[]):void {
  ...
  parent.querySelector('#draft-help')!.textContent=all?`Clic droit : déplacement. Maj : ajouter à la file. ${pawns.reduce((n,p)=>n+(p.draft?.queue.length??0),0)} déplacement(s) en attente. Les besoins continuent. Tirs à venir.`:pawns.some(p=>p.draft)?'Sélection mixte : mobiliser tout le groupe pour un déplacement commun.':'';
}
