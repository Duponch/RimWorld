...
export class PresentationChanges {
  ...
  capture(world:World):boolean {
    const signature=JSON.stringify([
      ...
      world.pawns.map(p=>[p.id,p.state,p.jobId,p.faction,p.hostilityResponse,p.draft?.holdFire,p.flee,p.melee,p.stun,p.shooting,p.stagger,!!p.draft,p.draft?.target,p.draft?.queue,p.equipmentTask?.itemId,p.equipmentTask?.action,p.equipmentDropPending,p.rescue,p.tend?.patientId,p.tend?.phase,p.feed?.patientId,p.feed?.phase,p.medicalSleep,p.interruptedCargo,p.need?.phase,p.need?.kind==='sleep'?p.need.bedId:undefined,
