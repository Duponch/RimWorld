...
import { withoutQueuedHaul } from './haul-reservations.ts';
...
export type HaulOrderTarget={type:'pile';pileId:number}|{type:'job';jobId:number};
...
export function queuedHaulReason(world:World,task:HaulTask):string|undefined {
  const view=withoutQueuedHaul(world,task),pile=world.piles.find(p=>p.id===task.sourcePileId);
  ...
  if(!target||same(target,pile.owner))return 'La destination a disparu ou coïncide avec la source.';
}
