...
export class WorkEnvironment extends LightEnvironment {
  ...
  production(station:Structure,worker:Cell):ProductionFactors {
    ...
    const requiredRole=isStove(station.kind)?'kitchen':station.kind==='stonecutter'||station.kind==='tailor-bench'||station.kind==='electric-tailor-bench'?'workshop':undefined;
...
export class WorkEnvironmentCache {
  ...
  read(world:World,light=this.readLight(world)):WorkEnvironment {
    ...
    for(const s of world.structures) {
      if(!['research-bench','tailor-bench','electric-tailor-bench','bed','stonecutter','table','table-square','table-long','horseshoes','fueled-stove','electric-stove'].includes(s.kind))continue;
      ...
      for(const id of ids){let v=scores.get(id);if(!v){v={beds:0,laboratory:0,workshop:0,kitchen:0,dining:0,recreation:0};scores.set(id,v);}
        if(s.kind==='bed')v.beds++;else if(s.kind==='research-bench')v.laboratory+=54;else if(s.kind==='stonecutter'||s.kind==='tailor-bench'||s.kind==='electric-tailor-bench')v.workshop+=27;else if(isStove(s.kind))v.kitchen+=28;else if(s.kind==='table'||s.kind==='table-square'||s.kind==='table-long')v.dining+=12;else v.recreation+=7;}
    }
