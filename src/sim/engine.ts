...
export function applyCommand(world: World, command: Command): CommandResult {
  ...
  if(result.ok){
    ...
    detachMissingBills(world);detachMissingGunBills(world);reconcileRepairs(world);reconcilePowerFlicks(world);
...
function applyCommandInternal(world: World, command: Command): CommandResult {
  ...
  if(command.type==='cancel-unfinished')return world.piles.some(p=>p.id===command.itemId&&p.gunWork)?cancelGunWork(world,command.itemId):cancelUnfinished(world,command.itemId);
  ...
  if(command.type==='bill-add'||command.type==='bill-update'||command.type==='bill-remove'||command.type==='bill-move') {
    const result=applyBillCommand(world,command,drops);if(result.ok){detachMissingBills(world);detachMissingGunBills(world);wakePlanners(world);refreshStock(world);}return result;
  }
  ...
  if (command.type === 'priority') {
    if (!['clean','firefight','warden','basic','hunt','research','patient','bedrest','doctor','mine', 'gather', 'build', 'haul', 'grow', 'cook', 'craft'].includes(command.work) || !Number.isInteger(command.value) || command.value < 0 || command.value > 4) return refusal('invalid-priority', 'La priorité doit être comprise entre 0 et 4.');
  ...
  world.jobs.push({ id: world.nextId++, kind: command.kind, ...(isConstruction(command)?{construction:command.kind==='grave'?'frame' as const:'blueprint' as const,...command.kind==='grave'||command.kind==='lay-floor'?{}:{material:command.material??'wood' as const}}:{}),...(command.kind==='lay-floor'?{floor:command.floor}:command.kind==='remove-floor'?{floor:world.tiles[command.z*world.width+command.x]!.floor}:{}), x: command.x, z: command.z, orientation: command.orientation ?? 0, footprint: 'standard', status: 'pending', reservedBy: null, progress: 0, escrow: { wood: 0, food: 0 } });
