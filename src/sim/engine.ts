...
function applyCommandInternal(world: World, command: Command): CommandResult {
  ...
  if (command.type === 'priority') {
    if (!['firefight','warden','basic','hunt','research','patient','bedrest','doctor','mine', 'gather', 'build', 'haul', 'grow', 'cook', 'craft'].includes(command.work) || !Number.isInteger(command.value) || command.value < 0 || command.value > 4) return refusal('invalid-priority', 'La priorité doit être comprise entre 0 et 4.');
...
export function stepWorld(world: World, ticks = 1, diagnostics?:import('./work-planner.ts').SearchStats): void {
  ...
  for (let step = 0; step < ticks; step++) {
    ...
    for (let offset = 0; offset < world.pawns.length; offset++) {
      ...
      if(isConstruction(job)&&(pawn.skills.construction.level<powerConstructionSkill(job.kind)||job.kind==='cooler'&&pawn.skills.construction.level<5||job.kind==='electric-stove'&&pawn.skills.construction.level<4||!constructionSupplied(world,job)||!constructionSiteFree(world,job,pawn.id))){releaseWork(world,pawn);continue;}
      if(job.kind==='mine') {
    ...
    reconcileFires(world);reconcilePowerFlicks(world);reconcilePower(world);reconcileOrders(world);reconcileWildlife(world);advanceCorpses(world,thermal);
