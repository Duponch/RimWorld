...
export interface TravelSegment { from:Cell; to:Cell; start:number; end:number }
export function startTravel(world:World,pawn:Pawn,next:Cell):void {
  ...
  const duration = TRAVEL_TICKS * edgeLength(pawn,next);
  pawn.motion={from:{x:pawn.x,z:pawn.z},to:{x:next.x,z:next.z},start,end:start+duration};
