...
export function validatePlayerOrders(world:World,version:number,shapesOnly=false):string[] {
  ...
  for(const pawn of world.pawns) {
    ...
    if(!orders||typeof orders!=='object'||Array.isArray(orders)||!(orders.active===null||typeof orders.active==='number'&&Number.isSafeInteger(orders.active)&&orders.active>0||version>=18&&orders.active==='haul'||version>=20&&orders.active==='cook')
    ...
    if(orders.active==='haul'?!pawn.haul||!['job','stockpile',...(version>=19?['fuel','aside']:[])].includes(pawn.haul.destination.type):orders.active==='cook'?!pawn.cooking:orders.active!==null&&orders.active!==pawn.jobId)errors.push('Active order has no matching job.');
...
function validQueuedHaul(value:unknown,version:number,world:World):boolean {
  ...
  if(version>=19&&d.type==='fuel')return id(d.structureId)&&d.forced===true&&(d.forCooking===undefined||version>=20&&d.forCooking===true)&&Object.keys(d).every(k=>['type','structureId','forced',...(version>=20?['forCooking']:[])].includes(k));
