...
export function validatePlayerOrders(world:World,version:number,shapesOnly=false):string[] {
  ...
  for(const pawn of world.pawns) {
    ...
    if(!orders||typeof orders!=='object'||Array.isArray(orders)||!(orders.active===null||typeof orders.active==='number'&&Number.isSafeInteger(orders.active)&&orders.active>0||version>=18&&orders.active==='haul'||version>=20&&orders.active==='cook'||version>=47&&orders.active==='tend'||version>=46&&orders.active==='rescue')
    ...
    if(orders.active==='haul'?!pawn.haul||!['job','stockpile',...(version>=19?['fuel','aside']:[])].includes(pawn.haul.destination.type):orders.active==='cook'?!pawn.cooking:orders.active==='tend'?!pawn.tend:orders.active==='rescue'?!pawn.rescue:orders.active!==null&&orders.active!==pawn.jobId)errors.push('Active order has no matching job.');
