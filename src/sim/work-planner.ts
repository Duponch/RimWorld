...
export function planWork(world: World, pawn: Pawn, getBlocked: NavigationGrid, occupied: ReadonlySet<number>, budget: SearchBudget): void {
  ...
  if (!selfCare && !selfTreatment && !tendable.length && !feedable.length && !patients.length && !cooking && !fires.length && !world.jobs.length && (!world.stockpiles.length || !world.piles.length && !world.packed.length || pawn.priorities.haul === 0)) { pawn.planCooldown = PLAN_INTERVAL; return; }
  if (!selfCare && !selfTreatment && !tendable.length && !feedable.length && !patients.length && !cooking && !fires.length && !world.jobs.length && !mayImproveFurnitureStorage(world) && !mayImproveStorage(world)) {pawn.planCooldown=PLAN_INTERVAL;return;}
  ...
  const first = ready[0];
  if (first && !selfCare && !selfTreatment && !tendable.length && !feedable.length && (!patients.length||pawn.priorities.doctor>first.priority) && (!cooking || productionRank>first.priority) && (pawn.priorities.haul === 0 || first.priority <= pawn.priorities.haul)
  ...
  for(const worker of world.pawns)if(worker.equipmentTask?.action==='equip'){const id=worker.equipmentTask.itemId;sourceReserved.set(id,(sourceReserved.get(id)??0)+1);}
  ...
  if (Number.isFinite(constructionHaulPriority(pawn)) && pawn.hunger > 20 && (!best || best.priority >= constructionHaulPriority(pawn))) {
    ...
    const destinations: { destination: HaulDestination; target: Cell & { kind?: JobKind }; priority: number; workPriority:number; wood: number; food: number; chunk?:number; steel?:number; component?:number; weapon?:number; medicine?:number; blocks?:number; items?:ReadonlyMap<ItemId,number>; reachable?: boolean }[] = [];
    for (const job of world.jobs) {
    ...
    if(pawn.priorities.haul>0)for (const zone of world.stockpiles) {
      ...
      if (capacity > 0 && (zone.filters.weapon || zone.filters.medicine || zone.filters.wood || zone.filters.food || zone.filters.chunk || zone.filters.component || zone.filters.steel || zone.filters.blocks)) destinations.push({ destination: { type: 'stockpile', stockpileId: zone.id }, target: zone, priority: zone.priority, workPriority:pawn.priorities.haul, weapon:zone.filters.weapon?1:0, medicine:zone.filters.medicine?capacity:0, wood: zone.filters.wood ? capacity : 0, food: zone.filters.food ? capacity : 0, chunk: zone.filters.chunk ? 1 : 0, component: zone.filters.component ? capacity : 0, steel: zone.filters.steel ? capacity : 0, blocks:zone.filters.blocks ? capacity : 0 });
    }
