...
export function advanceBeautyNeeds(world:World,topology:RoomTopology):void {
  ...
  const map=cache.read({width:world.width,height:world.height,tiles:world.tiles,structures,filth:world.filth?.items,objects});
  const view=new RoomBeautyCapture(map,topology),step=BEAUTY_NEED_INTERVAL*24/TICKS_PER_DAY;
