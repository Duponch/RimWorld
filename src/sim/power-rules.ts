...
export const isPowerTrader=(kind:unknown):boolean=>kind==='wood-generator'||kind==='solar-generator'||kind==='wind-turbine'||kind==='heater'||kind==='standing-lamp'||kind==='cooler'||kind==='electric-stove'||kind==='electric-tailor-bench';
export const isElectrical=(kind:unknown):boolean=>isPowerTrader(kind)||kind==='battery'||kind==='power-conduit'||kind==='power-switch';
export const isFlickable=(kind:unknown):boolean=>kind==='wood-generator'||kind==='standing-lamp'||kind==='cooler'||kind==='heater'||kind==='electric-stove'||kind==='power-switch';
...
export const powerDemand=(s:Structure):number=>s.kind==='electric-stove'?350:s.kind==='electric-tailor-bench'?120:s.kind==='cooler'?(s.cooler?.high?200:20):s.kind==='heater'?(s.heater?.high?175:17.5):s.kind==='standing-lamp'?LAMP_DEMAND:0;
