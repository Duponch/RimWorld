...
export interface CorpseState {animalId:number;species:'hare';sex:'female'|'male';health:MedicalRecord;facing?:number}
...
export function corpseYield(p:MaterialPile):{meat:number;leather:number} {
  ...
  const h=c.health,coverage=HARE_MODEL.parts.reduce((sum,part,i)=>sum+(h.missing.some(m=>isWithinPart(part.id,m.part,HARE_MODEL))?0:HARE_MODEL.coverage[i]!),0);
  ...
  return {meat:curve(140*.2*coverage*injury),leather:curve(40*.2*coverage*injury)};
}
...
function corpsePile(a:WildAnimal,owner:MaterialOwner,rot:RotState):MaterialPile {
  return {id:a.id,kind:'corpse',item:'hare-corpse',quantity:1,owner,rot,
...
export function advanceCorpses(w:World,layout?:ThermalLayout):void {
  ...
  for(const a of w.wildlife.animals) {
    ...
    if((a.motion?.end??0)>w.tick||w.piles.length>=32768||groundCapacity(w,a,'hare-corpse')<1)continue;
