...
export function reconcileTending(world:World):void {
  for(const d of world.pawns)if(d.tend){const p=world.pawns.find(p=>p.id===d.tend!.patientId);
    if(tendingReason(world,d,p,true)||d.priorities.doctor===0&&d.orders.active!=='tend'||!p||Math.abs(d.tend.spot.x-p.x)+Math.abs(d.tend.spot.z-p.z)!==1||!canStandAt(world,d.tend.spot))releaseWork(world,d);
  }
}
export function processTending(world:World,doctor:Pawn,context:NeedContext,light:()=>number):void {
  ...
  if(target.injuryId!==undefined)tendInjury(patient.health!,target.injuryId,dryTendQuality(medicalTendQuality(doctor),healthRandom(world)));
  else tendMissingPart(patient.health!,target.part);
  reconcilePawnHealth(world,patient);
  context.event(`${doctor.name} a traité une plaie de ${patient.name} sans médicament.`);
