...
export function validatePower(world:World,version:number):string[] {
  ...
  for(const s of [...world.structures,...(world.packed??[]).map(p=>p.building)]) {
    ...
    if(version<42||!p||typeof p!=='object'||Array.isArray(p)||typeof p.on!=='boolean'||s.orientation!==0
    ...
    if(s.kind==='standing-lamp'&&(p.on&&p.parentId===null||p.parentId!==null&&!validPowerParent(topology,s,p.parentId)))errors.push('Invalid electrical parent.');
  }
