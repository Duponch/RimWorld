...
export function validatePower(world:World,version:number):string[] {
  ...
  for(const s of [...world.structures,...(world.packed??[]).map(p=>p.building)]) {
    ...
    if(s.kind==='machining-table'&&version<101||version<42||s.kind==='electric-stove'&&version<84||s.kind==='electric-tailor-bench'&&version<90||['battery','solar-generator','power-conduit','power-switch'].includes(s.kind)&&version<85||['heater','wind-turbine'].includes(s.kind)&&version<87||!p||typeof p!=='object'||Array.isArray(p)||typeof p.on!=='boolean'||s.kind!=='cooler'&&s.kind!=='electric-stove'&&s.kind!=='electric-tailor-bench'&&s.kind!=='machining-table'&&s.kind!=='battery'&&s.kind!=='wind-turbine'&&s.orientation!==0
      ||s.footprint!=='standard'||(s.kind==='electric-tailor-bench'?!['wood','steel'].includes(s.material??''):s.material!=='steel')||Object.keys(p).some(k=>!['on','parentId',...(version>=85&&isFlickable(s.kind)?['switchOn']:[])].includes(k))
