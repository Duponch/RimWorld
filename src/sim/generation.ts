...
export function generateWorld(seed: number, width: number, height: number): World {
  ...
  const world: World = { schemaVersion: 12, restRules: 'adult', spoiled: emptySpoilage(), foodRules: 'adult', seed: seed >>> 0, rng: (seed >>> 0) || 0x9e3779b9,
  ...
  for (const [offset, name] of ['Ada', 'Noé', 'Mina'].entries()) {
    world.pawns.push({ schedule: defaultSchedule(), restZeroTicks: 0, collapsePending: false, id: world.nextId++, name, x: cx + offset - 1, z: cz, hunger: 90 - offset * 5,
