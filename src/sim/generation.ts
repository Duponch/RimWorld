...
export function generateWorld(seed: number, width: number, height: number): World {
  ...
  const world: World = { schemaVersion: 25, packed:[],deconstructed: { count: 0, lostWood: 0, fuelTicks: 0 }, foodPolicies: initialFoodPolicies(), nextFoodPolicyId: 5, restRules: 'adult', spoiled: emptySpoilage(), foodRules: 'adult', seed: seed >>> 0, rng: (seed >>> 0) || 0x9e3779b9,
  ...
  for (const [offset, name] of ['Ada', 'Noé', 'Mina'].entries()) {
    world.pawns.push({ recreation: initialRecreation(50 + sample(world.seed, offset, 0, 101)*10), foodPolicyId: 1, schedule: defaultSchedule(), restZeroTicks: 0, collapsePending: false, id: world.nextId++, name, x: cx + offset - 1, z: cz, hunger: 90 - offset * 5,
      rest: 90 - offset * 3, mood: 80, comfort: 50, memories: [], orders: {active:null,queue:[]}, jobId: null, haul: null, cooking: null, need: null, bedId: null, needCooldown: 0, state: 'idle', priorities: { gather: 2, build: 2, haul: 3, grow: 2, cook: 2 },
