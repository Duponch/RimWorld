import { FEED_TICKS,feedingReason,feedingPlaceValid } from './feeding-rules.ts';
...
export function validateFeeding(w:World):string[] {
  ...
  for(const d of w.pawns)if(d.feed){
    ...
    if(feedingReason(w,d,p,true)||d.priorities.doctor===0&&d.orders.active!=='feed')errors.push('Invalid feeding patient or reservation.');
    ...
    if(d.recreation.task||d.tend||d.rescue||d.need||d.haul||d.cooking||d.jobId!==null)errors.push('Feeding conflicts with another activity.');
  }
  return errors;
}
