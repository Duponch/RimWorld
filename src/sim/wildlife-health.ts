...
export const animalBody=(a:WildAnimal)=>{
  if(a.health)return assessMedical(a.health);
  let body=HEALTHY_ANIMALS.get(a.species);if(!body){body=assessBody(HEALTHY_BODY_INPUT,animalBodyModel(a.species));HEALTHY_ANIMALS.set(a.species,body);}return body;
};
...
export function advanceAnimalHealth(w:World,a:WildAnimal):void {
  if(w.schemaVersion>=84&&a.food<=0&&a.state!=='dead')a.health??={...createMedicalRecord(Math.max(0,w.tick-1)),body:a.species};
