...
export interface PawnSkills { social?:SkillRecord; construction:SkillRecord; medicine:SkillRecord; shooting:SkillRecord; melee:SkillRecord; lastResetTick:number }
...
export function tickSkills(world:World,pawn:Pawn):void {
  ...
  if(world.tick%TICKS_PER_DAY<TICKS_PER_DAY/24&&(skills.lastResetTick<0||world.tick-skills.lastResetTick>=TICKS_PER_DAY/2)) {
    skills.lastResetTick=world.tick;skills.construction.dailyXp=0;skills.medicine.dailyXp=0;skills.shooting.dailyXp=0;skills.melee.dailyXp=0;if(skills.social)skills.social.dailyXp=0;
  }
  for(const skill of [skills.construction,skills.medicine,skills.shooting,skills.melee,...skills.social?[skills.social]:[]]){const loss=decay[skill.level-10];if(loss)learnSkill(skill,-loss);}
}
