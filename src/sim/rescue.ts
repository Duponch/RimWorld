...
export function startRescue(world:World,actor:Pawn,proposal:{patientId:number;bedId:number;path:Cell[]},forced=false):void {
  ...
  actor.rescue={patientId:proposal.patientId,bedId:proposal.bedId,phase:'approach'};
...
export function reconcileRescues(world:World):void {
  for(const actor of world.pawns)if(actor.rescue){
    ...
    if(!patient||!wantsRescue(patient)||!bed||!rescueBedAvailable(world,bed,patient,actor.id)||medicalWorkRefusal(actor))releaseWork(world,actor);
  }
}
export function processRescue(world:World,actor:Pawn,context:NeedContext):void {
  ...
  if(!patient||!wantsRescue(patient)||!bed||!rescueBedAvailable(world,bed,patient,actor.id)){releaseWork(world,actor);return;}
  actor.state='moving';
  ...
  context.event(`${actor.name} a installé ${patient.name} dans un lit${bed.medical?' médical':''}.`);
}
