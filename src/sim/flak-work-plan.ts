...
export function planFlakWork(world:World,pawn:Pawn,station:Structure,bill:CookingBill,reach:Reachability,budget:{pairs:number}):{handled:boolean;plan?:CookingPlan} {
  ...
  const candidates=bound?[bound]:world.piles.filter(p=>p.flakWork?.recipe===bill.recipe&&!p.flakWork.billId&&p.flakWork.parts.every(part=>bill.filters[part.item])&&p.owner.type==='ground'&&(p.owner.x-station.x)**2+(p.owner.z-station.z)**2<=bill.radius**2)
  ...
  for(const pile of candidates){
    if(pile.flakWork!.authorId!==pawn.id||pile.owner.type!=='ground'||reservedSource(world,pile.id)>0)continue;
