...
/** A bound workpiece waits for its author. An unbound piece can resume only
 * under a compatible recipe and material filter. */
export function planArtWork(world: World,pawn: Pawn,station: Structure,bill: CookingBill,reach: Reachability,budget: {pairs:number}): {handled:boolean;plan?:CookingPlan} {
  if (!isArtRecipe(bill.recipe)) return {handled:false};
  const bound = world.piles.find(p => p.artWork?.billId === bill.id), spot = cookingSpot(station);
  const candidates = bound ? [bound] : world.piles.filter(p => p.artWork?.recipe === bill.recipe && !p.artWork.billId
    && bill.filters[p.artWork.material] && p.owner.type === 'ground'
    && (p.owner.x-station.x)**2+(p.owner.z-station.z)**2 <= bill.radius**2)
    .sort((a,b) => ((a.owner as Cell).x-pawn.x)**2+((a.owner as Cell).z-pawn.z)**2
      - (((b.owner as Cell).x-pawn.x)**2+((b.owner as Cell).z-pawn.z)**2) || a.id-b.id);
  for (const pile of candidates) {
    if (pile.artWork!.authorId !== pawn.id || pile.owner.type !== 'ground' || reservedSource(world,pile.id) > 0) continue;
    if (budget.pairs-- <= 0) {budget.pairs=0;return {handled:true};}
    const source = pile.owner, path = routeToJob(world,source,reach,true);
    if (!path) continue;
    const placed = ingredientPlaceFree(world,source,spot,bill.recipe,station);
    const cell = placed ? source : [station,{x:spot.x-1,z:spot.z},{x:spot.x+1,z:spot.z},
