...
export function queryCookingBillStatus(world:World,station:Structure,bill:CookingBill):{code:string;reason:string} {
  ...
  if(!productionResearchUnlocked(world,bill.recipe))return {code:'research-required',reason:'Recherchez Armurerie pour fabriquer cette arme.'};
  ...
  if(!world.pawns.some(p=>p.priorities[stationWork(station)]>0&&productionWorkerQualified(p,bill.recipe)))return {code:'skill-required',reason:`Un artisan de niveau ${isGunRecipe(bill.recipe)?GUN_REQUIREMENTS[bill.recipe].skill:0} est nécessaire.`};
