...
export type ResearchProject='complex-clothing'|'air-conditioning'|'batteries'|'solar-power';
export interface ResearchProgress {points:number;completedAt?:number}
export interface ResearchState extends ResearchProgress {project:ResearchProject|null;airConditioning?:ResearchProgress;batteries?:ResearchProgress;solarPower?:ResearchProgress}
...
export const projectProgress=(s:ResearchState,project:ResearchProject):ResearchProgress=>project==='complex-clothing'?s:project==='air-conditioning'?(s.airConditioning??={points:0}):project==='batteries'?(s.batteries??={points:0}):(s.solarPower??={points:0});
export const researchCost=(project:ResearchProject):number=>project==='complex-clothing'?CLOTHING_RESEARCH_COST:project==='air-conditioning'?AIR_CONDITIONING_COST:project==='batteries'?BATTERIES_RESEARCH_COST:SOLAR_POWER_RESEARCH_COST;
export const researchUnlocked=(w:World,project:ResearchProject):boolean=>project==='complex-clothing'?clothingUnlocked(w):project==='air-conditioning'?airConditioningUnlocked(w):project==='batteries'?batteriesUnlocked(w):solarPowerUnlocked(w);
...
export function selectResearch(world:World,project:unknown):CommandResult {
  if(project!==null&&project!=='complex-clothing'&&project!=='air-conditioning'&&project!=='batteries'&&project!=='solar-power')return {ok:false,code:'invalid-command',reason:'Projet inconnu.'};
...
export function processResearch(world:World,pawn:Pawn,move:(target:Cell,exact:boolean)=>unknown,rate:(s:Structure)=>number,event:(text:string)=>void):void {
  ...
  if(progress.points===cost){state.project=null;progress.completedAt=world.tick;
    ...
    event(project==='complex-clothing'?'Recherche achevée : Vêtements complexes. Établi de tailleur et chemise débloqués.':project==='air-conditioning'?'Recherche achevée : Climatisation. Climatiseur électrique débloqué.':project==='batteries'?'Recherche achevée : Batteries. Stockage électrique débloqué.':'Recherche achevée : Panneaux solaires. Production solaire débloquée.');
