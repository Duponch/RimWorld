...
export function validateDeconstruction(world: World, version: number, shapesOnly=false): string[] {
  ...
  if (!ledger || typeof ledger !== 'object' || Array.isArray(ledger)
    ...
    || Object.keys(ledger).some(k => !['count', 'lostWood', 'fuelTicks'].includes(k))) errors.push('Invalid deconstruction ledger.');
  for (const j of world.jobs) {
    ...
    if (!d || typeof d !== 'object' || Array.isArray(d) || !Number.isSafeInteger(d.structureId)
      || !Object.hasOwn(STRUCTURE_DEFINITIONS, d.kind) || Object.keys(d).some(k => !['structureId', 'kind'].includes(k))) {
      errors.push('Invalid deconstruction target.'); continue;
    }
    const s = deconstructionTarget(world, j);
    if (!s || s.kind !== d.kind || s.x !== j.x || s.z !== j.z || s.orientation !== j.orientation || s.footprint !== j.footprint
