...
import { equippedWeapon,WEAPON_QUALITIES } from './equipment-rules.ts';
import { pawnBody } from './health-rules.ts';
import { partMissing } from './injury-state.ts';
import type { Pawn,World } from './types.ts';

export type MeleeDamage='blunt'|'poke'|'bite';
export type MeleeToolId='left-fist'|'right-fist'|'head'|'teeth'|'grip'|'barrel'|'barrel-poke';
...
/** Natural adult and the current revolver only. Qualities affect melee damage;
...
export function meleeTools(world:World,pawn:Pawn,body=()=>pawnBody(pawn)):MeleeTool[] {
  ...
  const add=(id:MeleeToolId,damage:number,kind:MeleeDamage,factor=1)=>tools.push({id,damage,kind,penetration:damage*.015,cooldownCore:120,weight:damage*(1+damage*.015)/2*factor});
  ...
  if(weapon?.weapon&&weapon.item==='revolver'&&!pawn.equipmentDropPending&&body().capacities.manipulation>0) {
