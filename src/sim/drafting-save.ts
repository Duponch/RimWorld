...
export function validateDrafting(world:World):string[] {
  ...
  for(const p of world.pawns)if(p.draft) {
    ...
    if(p.path.length&&!p.transitExit&&(!d.target||!sameCell(p.path.at(-1)!,d.target)))errors.push('Tactical path does not reach its destination.');
    if(!d.target&&p.state==='moving'&&!p.transitExit&&p.moveCooldown===0)errors.push('Tactical movement lacks a destination.');
  }
  return errors;
}
