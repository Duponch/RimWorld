...
export function validateWorkProgress(world:World,version:number):string[] {
  ...
  for(const j of world.jobs) {
    ...
    if(j.pickTicks!==undefined&&(version<37||j.kind!=='mine'||!Number.isSafeInteger(j.pickTicks)||j.pickTicks<100||j.pickTicks>125))errors.push('Invalid captured mining pick duration.');
