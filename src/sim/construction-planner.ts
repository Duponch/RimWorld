...
export interface ConstructionCandidate {
  ...
  job?:Job; clearance?:{resourceId:number;progress:number};
...
export function constructionCandidates(world:World,pawn:Pawn,blocked:Uint8Array,reach:Reachability,budget:SearchBudget,obstacles:ReadonlyMap<number,ConstructionObstruction>):ConstructionCandidate[] {
  ...
  for(const job of world.jobs) {
    ...
    if(plant) {
      ...
      if(pawn.priorities.build>0&&plant.kind!=='rock'&&canReach(world,target,reach,false))result.push({...base,priority:pawn.priorities.build,target,job,clearance:{resourceId:plant.id,progress:0}});
