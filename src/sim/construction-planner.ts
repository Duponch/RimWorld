...
export function constructionCandidates(world:World,pawn:Pawn,blocked:Uint8Array,reach:Reachability,budget:SearchBudget,obstacles:ReadonlyMap<number,ConstructionObstruction>,jobs:readonly Job[]=world.jobs):ConstructionCandidate[] {
  ...
  for(const job of jobs) {
    ...
    } else if(job.construction==='frame'&&(job.kind!=='cooler'||pawn.skills.construction.level>=5)&&(job.kind!=='electric-stove'||pawn.skills.construction.level>=4)&&pawn.priorities.build>0&&constructionSupplied(world,job)&&constructionSiteFree(world,job,pawn.id,obstacle)&&canReach(world,job,reach,false)) {
