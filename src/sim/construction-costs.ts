...
export function frameAt(world: World, cell: Cell): boolean {
  return world.jobs.some(j=>j.construction==='frame'&&j.kind!=='lay-floor'&&footprintContains(j,cell));
}
...
export function frameCosts(world: World): ReadonlyMap<number,number>|undefined {
  ...
  for(const job of world.jobs)if(job.construction==='frame'&&job.kind!=='lay-floor') {
