...
export interface EquipmentTask {itemId:number;action:'equip'|'drop';progress:number;automatic?:true}
export type EquipmentCommand={type:'order-equipment';pawnId:number;itemId:number;action:'equip'|'drop';queue:boolean}
  |{type:'weapon-permission';itemId:number;allowed:boolean}|{type:'forget-weapon';pawnId:number};
