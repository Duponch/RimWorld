...
export function validatePawnHealth(world:World):string[] {
  ...
  for(const p of world.pawns) {
    ...
    if((stopped||pawnBody(p).capacities.manipulation===0)&&(p.firefighting||p.hunting||p.research||p.equipmentTask||p.jobId!==null||p.ward||p.feed||p.tend||p.rescue||p.haul||p.cooking||p.orders.active!==null||p.orders.queue.length||p.priorityWork))errors.push('Incapacitated pawn still owns work.');
