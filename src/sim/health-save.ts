...
export function validatePawnHealth(world:World):string[] {
  ...
  for(const p of world.pawns) {
    if(p.medicalSleep&&(p.state!=='downed'||p.moveCooldown>0))errors.push('Invalid medical sleep posture.');
    const h=p.health;
    ...
    if((stopped||pawnBody(p).capacities.manipulation===0)&&(p.jobId!==null||p.rescue||p.haul||p.cooking||p.orders.active!==null||p.orders.queue.length||p.priorityWork))errors.push('Incapacitated pawn still owns work.');
