...
import { GUN_REQUIREMENTS,isGunRecipe,type ProductionRecipe } from './production-recipes.ts';
import type { Pawn,World } from './types.ts';

export const productionResearchUnlocked=(world:World,recipe:ProductionRecipe):boolean=>!isGunRecipe(recipe)||gunsmithingUnlocked(world);
export const productionWorkerQualified=(pawn:Pawn,recipe:ProductionRecipe):boolean=>!isGunRecipe(recipe)||craftingSkill(pawn).level>=GUN_REQUIREMENTS[recipe].skill;
