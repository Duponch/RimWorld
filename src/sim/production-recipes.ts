...
export type ProductionIngredient = 'rice'|'berries'|'hare-meat'|'potato'|'corn'|'hare-corpse'|'cloth'|'unfinished-tribalwear'|'unfinished-shirt'|StoneIngredient;
...
export const stationRecipe=(station:Pick<Structure,'kind'>):ProductionRecipe|null=>isButcherStation(station.kind)?'butcher-creature':station.kind==='tailor-bench'?'shirt':station.kind==='crafting-spot'?'tribalwear':station.kind==='campfire'||isStove(station.kind)?'simple-meal':station.kind==='stonecutter'?'stone-blocks':null;
