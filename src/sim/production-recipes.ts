...
export type ProductionIngredient = 'rice'|'berries'|'cloth'|'unfinished-tribalwear'|'unfinished-shirt'|StoneIngredient;
export type ProductionRecipe = 'simple-meal'|'stone-blocks'|'tribalwear'|'shirt';
...
export const stationRecipe=(station:Pick<Structure,'kind'>):ProductionRecipe|null=>station.kind==='tailor-bench'?'shirt':station.kind==='crafting-spot'?'tribalwear':station.kind==='campfire'?'simple-meal':station.kind==='stonecutter'?'stone-blocks':null;
...
export function recipeProduct(recipe:ProductionRecipe,ingredients:readonly {item:ProductionIngredient}[]):ItemId {
  return recipe==='shirt'?'cloth-shirt':recipe==='tribalwear'?'cloth-tribalwear':recipe==='simple-meal'?'simple-meal':blockFor(ingredients[0]!.item as StoneIngredient);
}
export function isRecipeProduct(recipe:ProductionRecipe,item:ItemId):boolean {
  return recipe==='shirt'?item==='cloth-shirt':recipe==='tribalwear'?item==='cloth-tribalwear':recipe==='simple-meal'?item==='simple-meal':ITEM_DEFINITIONS[item].kind==='blocks';
}
