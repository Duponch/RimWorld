...
export function blockParts(x:number,z:number,item:ItemId,quantity:number):Placement[] {
  return Array.from({length:Math.min(6,Math.ceil(quantity/15))},(_,i)=>({x:x+(i%2? .17:-.17),z,y:.1+Math.floor(i/2)*.2,sx:.32,sy:.18,sz:.5,color:stonePileColor(ITEM_DEFINITIONS[item].color,x,z,i)}));
}
