...
export function researchTailorParts(world:World):Placement[]{
  ...
  for(const s of world.structures)if(s.kind==='research-bench'||s.kind==='tailor-bench'){
    ...
    else{add(-.6,height+.07,.03,.75,.14,.52,0xc4b794);add(.63,height+.12,.05,.22,.24,.22,0x867c63);add(.85,height+.025,-.16,.43,.05,.09,0xa4b4b0);}
  }return parts;
}
