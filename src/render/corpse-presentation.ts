...
export function corpseParts(x:number,z:number,stage:CorpseStage='fresh',yaw=0,species='hare'):Placement[] {
  const c=Math.cos(yaw),s=Math.sin(yaw);
  return animalParts(species).map(p=>({x:x+p.center[0]*c+p.center[2]*s,y:p.center[1]*.5,z:z+p.center[2]*c-p.center[0]*s,
