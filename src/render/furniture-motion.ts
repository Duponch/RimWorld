...
export function furnitureSurfaces(world:World):ReadonlyMap<number,number> {
  ...
  for(const s of world.structures) {
    const y=(isFoodWorkstation(s.kind)||s.kind==='stonecutter'||s.kind==='research-bench'||s.kind==='tailor-bench')?WORLD_SCALE.stonecutterHeight:s.kind==='table'?WORLD_SCALE.tableHeight:s.kind==='bed'?WORLD_SCALE.bedSurfaceHeight:s.kind==='stool'?WORLD_SCALE.stoolHeight:0;
