...
export function mergedInstances(group: THREE.Group, parts: { geometry: THREE.BufferGeometry; items: Placement[] }[], mat: THREE.Material, shadows = true): THREE.Mesh | undefined {
  ...
  const ranges: { id: number; start: number; count: number }[] = [];
  for (const { geometry, items } of parts) {
    ...
    for (const item of items) {
      ...
      if (item.key !== undefined) ranges.push({ id: item.key, start: firstIndex, count: index - firstIndex });
      vertex += pos.count;
    }
    geometry.dispose();
  }
  ...
  if (ranges.length) mesh.userData.resourceRanges = { ranges, original: indices.slice() };
  group.add(mesh);
  return mesh;
}
