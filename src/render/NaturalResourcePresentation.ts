...
type Shape=Pick<Resource,'id'|'kind'|'x'|'z'|'stone'>&{ripe:boolean;leafless:boolean};
...
export class NaturalResourcePresentation {
  ...
  read(world:World,reset=false):World|undefined {
    ...
    for(const r of world.resources)if(!isCrop(r)){
      ...
      if(!old||old.id!==r.id||old.kind!==r.kind||old.x!==r.x||old.z!==r.z||old.stone!==r.stone||old.ripe!==(r.kind==='berries'&&harvestable(world,r))||old.leafless!==plantLeafless(world,r))changed=true;
    }
    if(!changed&&index===this.shapes.length)return;
    const natural=world.resources.filter(r=>!isCrop(r));
    this.shapes=natural.map(r=>({id:r.id,kind:r.kind,x:r.x,z:r.z,stone:r.stone,leafless:plantLeafless(world,r),ripe:r.kind==='berries'&&harvestable(world,r)}));
