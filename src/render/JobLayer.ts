...
export function buildJobMarkers(world: World, group: THREE.Group, cutaway: boolean, batches: BoxBatches): void {
    ...
    for (const job of world.jobs.filter(j=>!isRoofJob(j))) {
      ...
      if(job.kind==='deconstruct'||job.kind==='uninstall') {
        ...
        const y=((isFoodWorkstation(targetKind)||targetKind==='machining-table'||targetKind==='art-bench'||targetKind==='research-bench'||targetKind==='tailor-bench'||targetKind==='stonecutter')?WORLD_SCALE.stonecutterHeight:(targetKind==='wall'||targetKind==='cooler')?wallHeight:targetKind==='table'?WORLD_SCALE.tableHeight:targetKind==='horseshoes'?WORLD_SCALE.horseshoeHeight:targetKind==='stool'?WORLD_SCALE.stoolHeight:WORLD_SCALE.bedSurfaceHeight)+.06;
      ...
      const height = kind==='solar-generator'?.56:kind==='power-conduit'?.08:kind==='power-switch'?.44:kind==='battery'?1.1:(isFoodWorkstation(kind)||kind==='machining-table'||kind==='art-bench'||kind==='research-bench'||kind==='tailor-bench'||kind==='stonecutter')?WORLD_SCALE.stonecutterHeight:kind === 'horseshoes' ? WORLD_SCALE.horseshoeHeight : (kind === 'wall'||kind==='cooler') ? wallHeight : kind === 'table' ? WORLD_SCALE.tableHeight : kind === 'stool' ? WORLD_SCALE.stoolHeight : WORLD_SCALE.bedSurfaceHeight;
      const width = kind==='solar-generator'?3.9:kind==='power-conduit'?.18:kind==='power-switch'?.7:kind==='battery'?.86:(isFoodWorkstation(kind)||kind==='machining-table'||kind==='art-bench'||kind==='research-bench'||kind==='tailor-bench'||kind==='stonecutter')?WORLD_SCALE.stonecutterWidth:kind === 'horseshoes' ? 0.12 : (kind === 'wall'||kind==='cooler') ? 0.92 : kind === 'table' ? WORLD_SCALE.tableWidth : kind === 'stool' ? WORLD_SCALE.stoolWidth : WORLD_SCALE.bedWidth;
      const length = kind==='solar-generator'?3.9:kind==='power-conduit'?.18:kind==='power-switch'?.7:kind==='battery'?1.83:kind==='research-bench'?1.8:(isFoodWorkstation(kind)||kind==='machining-table'||kind==='art-bench'||kind==='tailor-bench'||kind==='stonecutter')?WORLD_SCALE.stonecutterDepth:kind === 'horseshoes' ? 0.12 : kind === 'table' ? WORLD_SCALE.tableLength : kind === 'stool' ? WORLD_SCALE.stoolWidth : kind === 'bed' && job.footprint !== 'legacy-single' ? WORLD_SCALE.bedLength : 0.92;
