...
export function buildJobMarkers(world: World, group: THREE.Group, cutaway: boolean, batches: BoxBatches): void {
    ...
    for (const job of world.jobs.filter(j=>!isRoofJob(j))) {
      ...
      if(job.kind==='deconstruct'||job.kind==='uninstall') {
        ...
        const y=((isFoodWorkstation(targetKind)||targetKind==='machining-table'||targetKind==='research-bench'||targetKind==='tailor-bench'||targetKind==='stonecutter')?WORLD_SCALE.stonecutterHeight:(targetKind==='wall'||targetKind==='cooler')?wallHeight:targetKind==='table'?WORLD_SCALE.tableHeight:targetKind==='horseshoes'?WORLD_SCALE.horseshoeHeight:targetKind==='stool'?WORLD_SCALE.stoolHeight:WORLD_SCALE.bedSurfaceHeight)+.06;
