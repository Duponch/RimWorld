...
export class ColonyRenderer {
  ...
  private updateHover(): void {
    ...
    const cells = footprintCells({ ...cell, kind: this.tool==='install'&&this.furniturePlacement?this.furniturePlacement.kind:this.tool === 'wood-generator' || this.tool === 'research-bench' || this.tool === 'tailor-bench' || this.tool === 'stonecutter' || this.tool === 'bed' || this.tool === 'table' || this.tool === 'stool' ? this.tool : 'wall', orientation: this.placementRotation });
    ...
    const validity = this.tool==='install'&&this.furniturePlacement?installCommand(this.world,{type:'install',structureId:this.furniturePlacement.id,...cell,orientation:this.furniturePlacement.kind==='standing-lamp'?0:this.placementRotation},true):this.tool === 'research-bench' || this.tool === 'tailor-bench' || this.tool === 'crafting-spot'||this.tool === 'wood-generator'||this.tool === 'standing-lamp'||this.tool === 'passive-cooler'||this.tool === 'door'||this.tool === 'research-bench' || this.tool === 'tailor-bench' || this.tool === 'stonecutter'||this.tool === 'mine'||this.tool === 'uninstall'||this.tool === 'wall' || this.tool === 'bed' || this.tool === 'table' || this.tool === 'stool' || this.tool === 'campfire' || this.tool === 'horseshoes' || this.tool === 'chop' || this.tool === 'harvest' || this.tool === 'cut'
      ? canDesignate(this.world, { type: 'designate', kind: this.tool, ...cell, ...(this.tool==='wood-generator'||this.tool==='standing-lamp'?{material:'steel' as const}:this.tool==='passive-cooler'?{material:'wood' as const}:{}), orientation: this.tool==='wood-generator'||this.tool==='standing-lamp'||this.tool==='door'||this.tool==='passive-cooler'?0:this.placementRotation }) : undefined;
  ...
  private onKeyDown = (event: KeyboardEvent): void => {
    ...
    if ((this.tool === 'install' || this.tool === 'bed' || this.tool === 'table' || this.tool === 'campfire' || this.tool === 'research-bench' || this.tool === 'tailor-bench' || this.tool === 'stonecutter' || this.tool === 'research-bench' || this.tool === 'tailor-bench' || this.tool === 'crafting-spot') && (key === 'q' || key === 'e')) return;
