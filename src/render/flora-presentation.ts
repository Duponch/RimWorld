...
export function appendFlora(parts:FloraParts,world:World,r:Resource,turn:number):void {
  ...
  if(r.kind==='tree') {
    const height=r.species==='saguaro'?2.2:r.species==='drago'?2.7:r.species==='pine'?4.6:r.species==='poplar'?4.3:3.8;
  ...
  } else if(r.species==='agave'||r.species==='grass'||r.species==='tall-grass') {
    const agave=r.species==='agave',h=r.species==='tall-grass'?.5:agave?.4:.22;
    for(let i=0;i<(agave?6:3);i++) {const angle=turn+i*Math.PI/3;add(parts.blades,{dx:Math.sin(angle)*.13,dz:Math.cos(angle)*.13,y:h*.5,sx:agave?.15:.08,sy:h,sz:agave?.65:.4,ry:angle});}
  } else {
