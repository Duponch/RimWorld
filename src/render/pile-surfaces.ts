...
export function pileSurfaces(world:World):ReadonlyMap<number,PileSurface> {
  ...
  for(const s of world.structures) {
    const surface=(isFoodWorkstation(s.kind)||s.kind==='stonecutter'||s.kind==='research-bench'||s.kind==='tailor-bench')?{x:0,y:WORLD_SCALE.stonecutterHeight,z:0,scale:.65}:s.kind==='table'?{x:0,y:WORLD_SCALE.tableHeight,z:0,scale:.8}
