...
const rotatableTools=new Set<Tool>(['machining-table','grave','wind-turbine','battery','fueled-stove','electric-stove','butcher-table','install','bed','table','table-square','table-long','dining-chair','armchair','end-table','dresser','campfire','stonecutter','butcher-spot','crafting-spot','research-bench','tailor-bench','electric-tailor-bench','cooler']);
...
function renderState() {
  ...
  if(selection.ids.size>1) {
    ...
    if (x >= world.width || z >= world.height) clearSelection();
      ...
      updateUnfinishedInspection(el('inspector'),world,piles.find(p=>p.unfinished||p.gunWork),c=>void attempt(()=>client.command(c)));
