...
const rotatableTools=new Set<Tool>(['art-bench','machining-table','grave','wind-turbine','battery','fueled-stove','electric-stove','butcher-table','install','bed','table','table-square','table-long','dining-chair','armchair','end-table','dresser','campfire','stonecutter','butcher-spot','crafting-spot','research-bench','tailor-bench','electric-tailor-bench','cooler']);
...
function pickCell(x: number, z: number) {
  ...
  if (currentTool !== 'select') {
    ...
    void attempt(() => {
      ...
      return client.command(tool === 'cancel' ? { type: 'cancel', x, z } : { type: 'designate', kind: tool as JobKind, ...(target?{targetId:target.id}:{}), orientation: tool==='solar-generator'||tool==='power-conduit'||tool==='power-switch'||tool==='wood-generator'||tool==='standing-lamp'||tool==='door'||tool==='passive-cooler'?0:placementOrientation, x, z, ...(constructionUI.material(tool) ? {material:constructionUI.material(tool)} : {}) });
    });
    return;
  }
