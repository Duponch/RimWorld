...
## Socle de requêtes de combat sous V53
- Lire docs/development/combat-queries.md et docs/research/combat-preparation.md. combat-space/combat-report sont isolés, sans adaptateur World ni tir jouable. Une probabilité ne prouve pas la ligne ; portée avant penchement, visibilité distincte de navigation, couverture et posture séparées des branches de projectile.
- Grille détenue par l'appelant pour une décision synchrone ; aucune réutilisation après mutation. Rapports sans mutation ni PRNG caché. Valeurs synthétiques des tests/bancs ne sont pas les définitions de contenu ; ne pas présenter les requêtes de cent candidats comme cent combattants intégrés.
- `ranged-statistics` prépare les qualités du revolver et le calcul adulte de précision, sans compétence Tir persistée ni attaque. Lire docs/research/ranged-statistics-reference.md : dommage arrondi et pénétration distincts ; vitesse brute /100 par tick Core, journée Core/locale facteur 10. Conserver les fractions de phase lors de l'intégration ; le cycle d'XP utilise la préparation de base et le cooldown flottant, pas les durées arrondies. Gunshot et ses propagations exigent encore un producteur anatomique et une migration stricte.
