...
## Modèles et économie de validation (21 septembre 2026)
- L'utilisateur a repris Astra en raisonnement très élevé et demande de réduire le coût total par livraison. Pour les sous-agents techniques (code, tests, audit de règles), choisir au minimum Sol ; ne pas confier ces contrats à Luna. Garder des missions bornées et des fichiers attribués, puis intégrer et vérifier au centre. Une délégation qui exige des reprises répétées n'est pas une économie.
- Conserver les résultats acquis et rejouer les contrats touchés par une correction, pas toute la suite par réflexe. Le gel des sources natives, les frontières métier, migrations strictes et mesures successives restent exigés. Le temps et les tokens déjà dépensés ne justifient aucune affirmation de validation sans preuve.
- Les pilotes UI utilisent les coûts des recettes réellement choisies et règlent tous les filtres de stockage, dont Argent. Charger une fixture historique immuable puis comparer tous ses champs conservés avec les ajouts de migration admis ; ne pas comparer les deux numéros de schéma comme s'ils devaient être identiques. Fermer le menu de sauvegarde avant un clic qu'il masque. Ces erreurs de pilote ne se corrigent pas en assouplissant les règles du jeu.

## Habitat et habillement V90
- Lire `habitat-comfort.md`, `apparel-renewal.md` et leurs recherches. Sept constructions, qualité conservée, confort depuis les meubles occupés, besoin personnel de beauté, hémérocalle physique ; cinq familles tissu/cuir, tailleur électrique, usure quotidienne et deux politiques. Art/impression, éditeur de politiques et diversité biologique restent partiels ou absents. La rotation des chaises/fauteuils/chevets est exposée dans Architecte.
- V89 validée avant migration neutre V90 : aucune usure passée, recherche ni matière inventée ; automatisation historique désactivée. Installations de chambre : distance entre centres, visibilité requise par défaut dans la classe locale Core 1.6.4871. Les gestes d’habillage conservent leurs durées de familles différentes et leurs réservations.
- Pilote commun : `tests/fixtures/colony-v90.json.gz` et son carnet, J182,577, quatre habitants. Reprises de la vraie colonie V89 ; les branches pourries restent distinctes. Prévoir une coupe au-delà des anneaux épuisés, traiter les dépouilles avant péremption, éviter les plans sur des corps sans destination et relire les coordonnées après replanification. Aucun retrait d’assertion de survie ou de conservation.
- Preuves : 49 contrôles ciblés finaux, 21 UI reprises, présentation, campagne de 21 jours avec checkpoints. CPU p95 79,65 ms ; natif image p95 33,4 ms, pic 112,5 ms, débit 2,324×/6× à cent colons/cent animaux. Charges non identiques à V89, aucun gain causal ni fluidité garantie. Mode jour : publication puis retour à l’utilisateur.

## Contenu et progression par domaines (après V88)
- L'utilisateur demande une estimation de chaque aspect et un enrichissement du contenu pendant les lots de mécanique : matériaux de construction, sols, meubles/décorations, ressources, ateliers/recettes, équipements, plantes/animaux et biomes. Ne pas différer tout le catalogue à une phase finale. ROADMAP garde les estimations incertaines, le calendrier et la balance entre domaines.
- Chaque lot précise les contenus réellement obtenables qu'il ajoute ou complète, leurs matières, moyens d'acquisition/production, usage physique, effets, stockage et sauvegarde. Une définition, une variante de couleur ou un menu grisé ne vaut pas contenu jouable. Le nombre d'objets définis n'est pas une mesure de parité.
- V89 livré dans son périmètre : lire `burial.md`, `cleanliness.md`, `food-poisoning.md` et leurs recherches. Corps humain unique avec identité conservée, tombe propriétaire suspendant l'âge, aucune position fantôme après transport. Salissures/propreté de pièce et nettoyage physiques ; sols issus de matériaux/recherche. Contamination fractionnelle conservée à chaque transfert, maladie distincte et vomissement non interruptible. V88 strictement validée avant migration neutre ; observations prospectives des anciens morts. Crashlanded révision 4 connaît Taille de pierre seulement à la création, jamais par migration.
- Pilote commun V89 : reprendre la fixture propre `colony-v89.json.gz` et son carnet. Conserver les accès du froid lorsque l'habitat s'étend ; le premier plan a enfermé Noé puis a été réparé par déconstruction et secours, sans retrait de l'assertion de survie. Preuves et limites dans `docs/history/validation-hygiene-v89.md`. Préparer plusieurs fronts suivants selon ROADMAP, sans enfermer le projet dans la seule salubrité. Validation groupée, intégration centrale et optimisations mesurées restent obligatoires.
