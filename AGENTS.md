...
- Lire docs/development/projectiles.md et sa recherche. Émission, permissions et vol restent hors World/UI ; aucune persistance de partie implicitement livrée par le JSON des fixtures. Cible intentionnelle/utilisée, arme du départ et lanceur distincts ; PRNG local engagé avec le résultat, jamais seul après refus.
...
- projectile-world capture les cibles actuelles, recouvrement complet par couche logique, patients/objets portés exclus et relations copiées explicites ; la capture du meilleur couvert reste distincte. Colonnes/incidences, identifiants ordonnés vérifiés puis recherche binaire ou index différé ; pas d'hypothèse 32 bits ou d'ordre World. Recréer après mutation et partager par lot. Dépouilles et dommages aux objets absents. Prochain branchement : phases et migration stricte, vraie commande/XP, observateur de présentation et adversaire, avec garde UI et charge mixte. Les coûts de lots de vols complets ne sont pas des coûts par frame.
